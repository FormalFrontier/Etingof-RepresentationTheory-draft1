...
- **Twisting an `sl(2)` (or any Lie-algebra) representation to make `ρ(e)` a chosen operator — conjugation recipe (#5309, single Jordan block of Jacobson–Morozov part (l), `Chapter2/Problem2_15_1_l.lean`).** To turn the irreducible `rhoLieHom n : sl2 →ₗ⁅ℂ⁆ Module.End ℂ (Fin n → ℂ)` into a rep whose `ρ(e)` is a *specific* nilpotent (here the standard shift `J_{0,n}`, `e_k ↦ e_{k-1}`), conjugate by a `LinearEquiv` `φ`: `(φ.conjAlgEquiv ℂ : _ →ₐ[ℂ] _).toLieHom.comp (rhoLieHom n)` is again a `LieHom` (algebra-equiv conjugation `LinearEquiv.conjAlgEquiv` from `Mathlib.Algebra.Algebra.Equiv` preserves the commutator bracket; `AlgHom.toLieHom` lifts it). `conjAlgEquiv_apply` rewrites it to `φ ∘ₗ f ∘ₗ φ.symm`. Since `rhoLieHom n sl2_e` acts as `e_k ↦ k · e_{k-1}` (a single Jordan block already), the diagonal rescaling `φ : e_k ↦ k! · e_k` normalises the subdiagonal coefficients to `1`. **Crucial API constraint: `Sl2Irrep.lean`'s component maps `rhoH/E/F` and `rhoLieHom_sl2_*_eq` are `private`** — you cannot name them from another file. Compute the conjugation on the standard basis instead via the *public* `lie_eq_rhoLieHom` (`⁅x,v⁆ = rhoLieHom d x v`) + `lie_sl2_e_e_basis`/`lie_sl2_f_e_basis`/`lie_sl2_h_e_basis` + `e_basis`, and prove the endomorphism equality with `(Pi.basisFun ℂ (Fin n)).ext` (`Pi.basisFun_apply : … = Pi.single k 1`, defeq `e_basis n k` — bridge with `change`, not `show`, to dodge the style linter). Nilpotency of the shift: a `jordanShift_pow_apply` induction (`(J^m v) k = if k+m<n then v⟨k+m⟩ else 0`) gives `J^n = 0`. **`omega` gotcha: it does NOT unfold `Fin.val` of an explicit `Fin.mk` (`↑⟨a,h⟩` stays opaque)** — feed the nat-level equality directly, e.g. `congrArg v (Fin.ext (show a = b by omega))`, where `a`,`b` are the *reduced* vals. The general nilpotent case (assemble arbitrary `A` over Jordan blocks) needs a Jordan-basis decomposition that is **not** in Mathlib (`JordanChevalley` is only the semisimple+nilpotent split) — **DONE** (#5312, sorry-free axiom-clean `exists_sl2Rep_of_nilpotent`, same file).

- **Jordan-basis decomposition of a nilpotent endomorphism from the PID structure theorem (#5312, the reusable recipe — Mathlib has no explicit nilpotent Jordan/cyclic decomposition).** To split an arbitrary nilpotent `A : End ℂ V` (fin-dim) into standard shift blocks: give `V` the `ℂ[X]`-module structure with `X` acting as `A` via **`Module.AEval' A`** (`AEval'.of A : V ≃ₗ[ℂ] AEval' A`, `X • of A m = of A (A m)`, `Module.Finite ℂ[X]` from `Module.Finite ℂ V`); nilpotency (`A^N = 0`) gives `Module.IsTorsion' (AEval' A) (Submonoid.powers X)` (`X^N • of A m = of A (A^N m) = 0`); then **`Module.torsion_by_prime_power_decomposition Polynomial.irreducible_X`** (the *prime-`X` specialisation* — avoids the generic `equiv_directSum_of_isTorsion`'s "primes are associated to `X`" bookkeeping) yields `AEval' A ≃ₗ[ℂ[X]] ⨁ i : Fin d, ℂ[X] ⧸ (ℂ[X] ∙ X^(k i))`. Compose with `DirectSum.linearEquivFunOnFintype` (⨁→Π) and `LinearEquiv.piCongrRight` over the per-block identification. **The block `ℂ[X] ⧸ (Xᵉ) ≃ₗ[ℂ] (Fin e → ℂ)` with `X ↦ jordanShift e`** is cleanest via the *reversed low-degree coefficient* map `p ↦ (fun k : Fin e => p.coeff (e-1-k))` (`LinearMap.pi ∘ lcoeff`) and its inverse `v ↦ ∑ⱼ monomial (e-1-j) (v j)`; both round-trips are pure coefficient arithmetic + `Polynomial.X_pow_dvd_iff` (`X^n ∣ f ↔ ∀ d<n, f.coeff d = 0`) — **no `aeval`-sum, no `finrank`/`AdjoinRoot`, and `e = 0` needs no special case**. Intertwining `blockEquiv (X • q) = jordanShift (blockEquiv q)` is `coeff_X_mul`/`coeff_X_mul_zero`. Two non-obvious levers: (1) **build the ℂ-linear map OUT of the ℂ[X]-quotient via `Submodule.liftQ ((span ℂ[X] {X^e}).restrictScalars ℂ) coeffMap hle`** — the ℂ-quotient type `ℂ[X] ⧸ (span…).restrictScalars ℂ` is *defeq* to the ℂ[X]-quotient the PID theorem produces, so `liftQ`/`mkQ`/`Submodule.Quotient.mk` all match by defeq; **but pass the submodule EXPLICITLY — an `_` placeholder times out at `isDefEq` (200000 heartbeats)** reconciling the restrictScalars/quotient defeq. (2) The **whole final ℂ-linear equiv is `(of A) ≪≫ₗ Ψ.restrictScalars ℂ ≪≫ₗ funOn.restrictScalars ℂ ≪≫ₗ piCongrRight blockEquiv`**, and the `A`↔`⨁ jordanShift` intertwining is one `simp only [LinearEquiv.trans_apply, restrictScalars_apply, piCongrRight_apply, LinearMap.coe_piMap, Pi.map_apply] ; rw [X_smul_of.symm, map_smul, map_smul, Pi.smul_apply, block_intertwine]` — because every middle equiv is ℂ[X]-linear (so commutes with `X•`), `X` acts as `A` on the left and the block shifts on the right, and the needed `IsScalarTower ℂ ℂ[X] _` / `Module ℂ _` instances on `AEval'`, `⨁`, `Π` all resolve automatically. Finally feed the intertwining to the assembly step (`exists_sl2Rep_of_conj_piShift`). `Fin.val`-of-`Fin.mk` in the coeff `omega` goals: a bare `simp only []` reduces `↑⟨e-1-d,_⟩` to `e-1-d` so `omega` then closes.
