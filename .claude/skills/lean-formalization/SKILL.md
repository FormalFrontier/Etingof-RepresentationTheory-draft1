...
| `↑(f.restrict h x) = f x` | `LinearMap.restrict_coe_apply` (f explicit) | `LinearMap.coe_restrict_apply` (f **implicit**) | Deprecated 2026-05-13 and the **argument structure flipped**: the new `coe_restrict_apply {f} (hf) (x)` takes `f` implicitly, so `coe_restrict_apply (localRightRep k n g) hf x` gives "Function expected at …" — drop the `f`, write `coe_restrict_apply hf x` (f infers from `hf`). Still defeq-bridges `S.toRepresentation g w` (= `(ρ g).restrict _ w`) to `↑(…)`, the workhorse for `Subrepresentation.toRepresentation` coercions. Old name still compiles with a warning; used across Ch5 (`RightTranslationHullDecomp.lean:98,280,291`). (#5602) |
