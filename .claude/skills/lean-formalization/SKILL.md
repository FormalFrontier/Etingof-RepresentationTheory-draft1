...
- **The Casimir operator is central on any `sl(2)`-module — `casimir_central` (Problem 2.15.1(g), #5311/#5315, sorry-free in `Chapter2/Problem2_15_1_complete_reducibility.lean`).** To prove `C = casimir M` commutes with the action (`⁅x, C m⁆ = C ⁅x, m⁆` for all `x : sl2`), work at the `Module.End ℂ M` level. **Required first step: `attribute [local instance 100] LieRing.ofAssociativeRing` in-section** — the `LieRing (Module.End ℂ M)` structure is only a *local* instance from Lean v4.31 (see `Sl2Defs.lean`), and without it `LieHom.map_lie` and the commutator bracket `Ring.lie_def` (`⁅a,b⁆ = a*b - b*a`) fail to synthesize. Recipe: (1) the bracket is a derivation of `*` — `lie_mul' : ⁅a, b*c⁆ = ⁅a,b⁆*c + b*⁅a,c⁆` by `simp only [Ring.lie_def]; noncomm_ring` (add `omit [LieRingModule sl2 M] [LieModule ℂ sl2 M] in` to silence the unused-section-var linter). (2) Per generator `g ∈ {e,f,h}`, prove `⁅toEnd g, casimir M⁆ = 0`: `rw [casimir]`, `set E/F/H := toEnd …`, get the three bracket relations as `End` equations from the sl₂-triple via `rw [← LieHom.map_lie, lie_e_f/lie_h_e/lie_h_f, map_nsmul/map_neg]` (and `(lie_skew E H).symm` for the reversed `⁅E,H⁆`), then `rw [lie_add, lie_add, lie_mul', lie_mul', lie_smul, lie_mul']` and close with `simp only [lie_self, <relations>, zero_mul, mul_zero, add_zero, zero_add, neg_mul, mul_neg, smul_mul_assoc, mul_smul_comm]` followed by **`module`** (handles the mixed ℕ-smul `2•` / ℂ-smul `2⁻¹•` arithmetic over the `E*H`, `H*E` atoms). (3) Reduce arbitrary `x` to the generators with `sl2_decomp : x = x.val 0 1 • sl2_e + x.val 1 0 • sl2_f + x.val 0 0 • sl2_h` (matrix-entry read-off, `Subtype.ext` + `push_cast` + `fin_cases` + `sl2_traceless`), pushed through `map_add`/`map_smul` + `add_lie`/`smul_lie`. (4) Convert `⁅toEnd x, C⁆ = 0` to the module statement via `Ring.lie_def`+`sub_eq_zero` (giving `toEnd x * C = C * toEnd x`) + `Module.End.mul_apply` + `LieModule.toEnd_apply_apply`. This is the prerequisite for part (h) (generalized eigenspaces of the central `C` are sub-`sl₂`-modules, #5316).
