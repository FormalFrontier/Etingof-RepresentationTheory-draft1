...
- **"Formal combination of the irreducibles" means indexed by `Etingof.IrrepClasses`, and that type is NOT in `Type 0` (#7483, `Infrastructure/IrrepClasses.lean` + `Chapter5/Definition5_7_1.lean`, sorry-free).** Whenever the book says "an integer/natural combination of *the* irreducibles `Σ nᵢ Vᵢ`" (Def 5.7.1, and anything else summing over a complete set of irreducibles), the index must be **isomorphism classes**, not literal `FDRep k G` objects: with literal objects, `+1` on one model plus `-1` on an isomorphic copy is nonzero coefficient data although it is zero in the representation group, and no `iso_invariance` *field* repairs this because literal-object finite support is already the wrong notion. Use `Etingof.IrrepClasses k G` (in `Infrastructure/IrrepClasses.lean` — Mathlib-only imports, deliberately *not* in `Chapter4/Exercise4_2_3.lean` where it was born) with `→₀ ℤ`; `abbrev`, not `def`, so the `Finsupp` `AddCommGroup`/basis API applies directly. Three traps: (a) **`Iso.mk ⟨e.hom, e.inv, …⟩` does NOT typecheck** to convert a `FullSubcategory` iso to an ambient one even though morphisms *are* the ambient morphisms — you get `Application type mismatch: e.hom has type P ⟶ Q but is expected to have type P.obj ⟶ Q.obj` (and the mirror image the other way), regardless of this repo's `weak.backward.isDefEq.respectTransparency = false`. Always go through `(ObjectProperty.ι _).mapIso e` and `ObjectProperty.isoMk _ e`. (b) `IrrepClasses ℂ G` lives in **`Type 1`** (objects of `FDRep` are bundled), so any pre-existing family-shaped theorem written with `{ι : Type} [Fintype ι]` cannot take the support-indexed family — universe-generalize it to `{ι : Type*}` (a strict generalization, zero downstream cost) rather than transporting through `Fin n`. (c) `repOf` is `Quotient.out`, hence opaque: the only facts about it are `mk_repOf` and `nonempty_repOf_iso_repOf_iff` (distinct classes have non-isomorphic representatives — this is exactly the `hdistinct : ∀ i j, Nonempty (W i ≅ W j) → i = j` hypothesis that family-shaped lemmas take). That is enough to feed class-indexed data to a family theorem via `Finset.sum_coe_sort`, but if you need a *canonical* irreducible per class (character tables) build an explicit enumeration instead. Descend `character`/`finrank` to classes with `Quotient.lift` + `FDRep.char_iso` (resp. the `LinearEquiv` under the iso) once, in the infra file, and state the virtual character as an `AddMonoidHom` via `Finsupp.liftAddHom (fun c => zmultiplesHom _ c.character)` so `map_add`/`map_neg`/`map_sub` are structural; recover the book's `Σ nᵢ χ_{Vᵢ}` with `Finsupp.liftAddHom_apply` + `Finset.sum_apply` + `zsmul_eq_mul` + `Pi.mul_apply`/`Pi.intCast_apply`.
...
- **`motive is not type correct` when you `rw` a *coercion of a subtype-valued constructor* whose proof-argument reappears in an anonymous constructor (#7911, `Chapter8/KoszulHomotopy.lean`; four of eight errors in one build cycle).** The `Set.powersetCard` idiom in the Koszul files carries membership proofs inside terms — `eraseElem s ⟨a, ha⟩`, `insertElem s hp.notMem` — and the goal routinely contains both `↑(insertElem s h)` and a `⟨a, hat⟩` whose `hat : a ∈ ↑(insertElem s h)`. Rewriting with `coe_insertElem` then fails: abstracting `↑(insertElem s h)` ill-types `hat`. **Do not rewrite the coercion. Two fixes, both one line:** (a) state the fact you want in **term mode** and let defeq unfold the coercion for you — `have herase : eraseElem (insertElem s h) ⟨p, hpt⟩ = s := Subtype.ext (Finset.erase_insert h)` typechecks because `↑(insertElem s h)` is defeq `insert p ↑s`, whereas the `by apply Subtype.ext; rw [coe_eraseElem, coe_insertElem]` version dies; (b) when the coercion sits under a function, `have` the *whole application* instead — `have hpos : finsetPos (insertElem s h : Finset κ) a = finsetPos (s : Finset κ) a + 1 := finsetPos_insert_of_mem …` — then `rw [hpos]`, which abstracts the `finsetPos …` node and leaves the coercion (and so `hat`) untouched. The same trap will hit #7912/#7883/#7884/#7886, which all manipulate these terms. Related but *not* the same as the `subst` fix above: there is nothing to `subst` here.

- **Two API-orientation slips from the same file, both cheap to pre-empt.** `Finsupp.single_eq_of_ne` wants the hypothesis the *other* way round from what reads naturally: for `(single a 1) x = 0` it expects `x ≠ a`, not `a ≠ x`. And **`le_or_lt` does not exist** — it is `le_or_gt`. Also: `omit [Inst] in` must come *before* a declaration's docstring, not between the docstring and the `theorem` keyword (otherwise: `unexpected token 'omit'; expected 'lemma'`).
...
- **Before hand-rolling *any* presented algebra, use `Etingof.PresentedAlgebra` (#7475, `Chapter2/Discussion_2_6.lean`).** The project now has the book's Discussion 2.6 construction once, generically: for `rel : R → FreeAlgebra k X`, `PresentedAlgebra k X R rel` is `k⟨X⟩` modulo `TwoSidedIdeal.span (Set.range rel)`, with `mk` (quotient `→ₐ[k]`), `mk_surjective`, `gen`/`gen_def`, `mk_rel : mk rel (rel r) = 0`, and the universal property `lift`/`lift_mk`/`lift_gen`/`hom_ext`/`existsUnique_lift` plus the converse `lift_rel_eq_zero`. The book's `k⟨x₁,…,xₙ⟩/⟨f₁,…,f_m⟩` is `X := Fin n`, `R := Fin m`. **This supersedes the `RingQuot Rel` half of the bullet above for the common case** where the relations are "these elements are zero": a family `rel` is far less machinery than an inductive `Rel` Prop, and `hom_ext` gives generator-wise uniqueness for free. Keep `RingQuot` only when the relation is genuinely a *pair* relation not of the form `f = 0` (rare here — `a = b` is `a - b = 0` in a ring), or when you need to match an existing `RingQuot`-based definition such as `Proposition2_7_1.WeylAlgebra`. Two API details that cost a build cycle each: `RingCon.mkₐ_surjective` takes its scalar ring as an **implicit** `α`, so write `(c.mkₐ_surjective (α := k))` and not a positional `k`; and do not tag your own `hom_ext` `@[ext]` when the carrier is an `abbrev` for `RingCon.Quotient`, since the attribute keys on the unfolded head and then fires on unrelated goals.
