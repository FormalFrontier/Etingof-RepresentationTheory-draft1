...
- **Landing the `finrank=3` eigenspace split (#5780 → PR #5819) — two gotchas that cost ~4 rebuilds:** (1) **`LinearMap.IsProj.trace`** (`= (finrank ℂ p : ℂ)`, the clean way to read a projection's rank) needs four instances Lean will NOT find automatically over the thrice-nested eigenspace subtype `↥(ker(z−μ⁺) ⊆ Λ²(ℂ⁴) ⊆ W4 ⊗ W4)`. Supply them by hand *inside* a `set_option synthInstance.maxHeartbeats 400000 in` (default 20000 times out on the deep subtype): `haveI : FiniteDimensional ℂ ↥lam2Sub.toSubmodule := FiniteDimensional.of_finrank_pos (by rw [lam2_finrank]; norm_num)` (the ambient carrier is NOT auto-`FiniteDimensional`; `↥lam2Sub.toSubmodule` is only defeq to `lam2`'s FDRep carrier, not syntactically it), then `haveI : Module.Free ℂ (↥(Module.End.eigenspace zEnd muPlus)) := Module.Free.of_divisionRing ℂ (↥(Module.End.eigenspace zEnd muPlus))` and the same for `↥(LinearMap.ker zProjPlus)`. **`Module.Free.of_divisionRing` takes `K` AND `V` explicitly** — automatic instance search silently declines to try it, and passing `_` for `V` leaves the metavariable blocking unification of the `Semiring ℂ` instance (`DivisionRing.toSemiring` vs `Complex.instSemiring`); write both out in full. (2) **`rw [mul_sub]` / `rw [map_sub]` / `rw [map_smul]` fail to keyed-match** on `Module.End` products and on the bundled `LinearMap.trace` functional ("Did not find an occurrence of the pattern `?a * (?b - ?c)`" / `?f (?a - ?b)`). Pass the operands explicitly — `rw [mul_sub zEnd zEnd (muMinus • 1)]`, `rw [LinearMap.map_sub (LinearMap.trace ℂ M) zEnd (muMinus • 1)]`, `rw [LinearMap.map_smul (LinearMap.trace ℂ M) c w]` — or use `simp only [mul_sub, map_sub, map_smul]` (the simp forms match fine). The apply-lemmas for `Module.End` are `Module.End.mul_apply` / `Module.End.one_apply` (NOT `LinearMap.mul_apply`/`LinearMap.one_apply`, which don't exist). Polynomial identities in `z` with a known quadratic relation close with `module` after substituting `hkey : z*z = (μ⁺+μ⁻)•z − (μ⁺·μ⁻)•1` (treats `z`,`1` as atoms); the coefficient-pinning `z²=20z+400` drops out of the 2×2 trace system by `linear_combination (c₁:ℂ)*e1 + (c₂:ℂ)*e2` over ℂ (no `linarith` — ℂ is unordered).
