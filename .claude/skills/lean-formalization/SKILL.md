...
- **Centre of `ℂ[G]` = class functions, and the renormalised-character recovery formula (Remark 4.5.3, #5336, sorry-free in `Chapter4/Remark4_5_3.lean`).** `classFunctions G := Subalgebra.center ℂ (MonoidAlgebra ℂ G)`. To prove `f ∈ centre ↔ IsClassFunction f` (`∀ x y, f(yxy⁻¹)=f x`): `simp only [classFunctions, Subalgebra.mem_center_iff]` (the `def`-name unfolds the centre membership to `∀ b, b*f = f*b`). **Forward**: test centrality against `single y 1`, evaluate the function equality at `y*x` via `congrArg (fun F => F (y*x)) (h (single y 1))`, then `simp only [single_mul_apply, mul_single_apply, one_mul, mul_one]` (these `MonoidAlgebra` group lemmas give `(single y 1 * f) p = f (y⁻¹*p)` and `(f * single y 1) p = f (p*y⁻¹)`) and clean the group word with `rw [show y⁻¹*(y*x) = x by group]`. **Backward**: `ext z; rw [mul_apply_left, mul_apply_right]` turns `b*f = f*b` into two `b.sum` expansions `∑ r·f(g⁻¹z)` vs `∑ f(zg⁻¹)·r`; `Finsupp.sum_congr` + `mul_comm` reduces to the pointwise conjugation `f(g⁻¹z)=f(zg⁻¹)`, supplied by the class-fn hyp at `(z*g⁻¹, g⁻¹)` after `rw [show g⁻¹*(z*g⁻¹)*g⁻¹⁻¹ = g⁻¹*z by group]`. `renormCharElt_mem_classFunctions` is then one `rw [mem_classFunctions_iff]` + `FDRep.char_conj`. The **recovery formula** `χ_V(g) = √(|G|/χ̃_V(1))·χ̃_V(g)` with normalisation `χ̃_V(z)=(χ_V(1)/|G|)χ_V(z)`: the witness is just `c = |G|/χ_V(1)` (both the `c²=|G|/χ̃_V(1)` and the `χ_V(g)=c·χ̃_V(g)` legs close with bare `field_simp` — no `ring` needed, it over-closes to "no goals"). Need `χ_V(1)=dim V≠0` for simple `V`: a universe-poly clone of `Corollary4_2_4.finrank_pos_of_simple` (`finrank=0 ⇒ Subsingleton V ⇒ Subsingleton (V⟶V)`, contradicting `FDRep.finrank_hom_simple_simple = 1`), then `FDRep.char_one` + `exact_mod_cast …ne'`. **`renormChar_isPrimitiveIdempotent` (#5336/#5349) is now DONE, sorry-free — and it needed NO Wedderburn `∏ℂ` infrastructure** (the earlier "primitivity needs the centre's `∏ℂ` structure, not in Mathlib" assessment was too pessimistic). The whole thing rests on one private **absorption lemma**: for simple `V` and *any* class function `z`, `∃ σ, renormCharElt V * z = σ • renormCharElt V`. Proof: the operator `S_z := ∑ y, z y • V.ρ y⁻¹ : ↑V →ₗ[ℂ] ↑V` is `G`-equivariant (`S_z ∈ (Representation.linHom V.ρ V.ρ).invariants`; the conjugation reindex uses `Fintype.sum_equiv ((Equiv.mulLeft g).trans (Equiv.mulRight g⁻¹))` + `IsClassFunction z`), and that invariant space is 1-dim for simple `V` (`Representation.linHom.invariantsEquivFDRepHom` + `CategoryTheory.finrank_endomorphism_simple_eq_one` + `finrank_eq_one_iff_of_nonzero'` with `id` nonzero — the scalar-extraction pattern of `Proposition4_7_1_ii`, factored as `exists_scalar_of_invariant`), so `S_z = σ • id`. The convolution coefficient identity `∑_x χ(x)·z(x⁻¹w) = σ·χ(w)` is then a reindex (`Fintype.sum_equiv ((Equiv.inv G).trans (Equiv.mulRight w))`, summand defeq via `change`) + trace computation `∑_y z y·χ(w y⁻¹) = tr(ρ(w) ∘ S_z) = tr(ρ(w) ∘ (σ•id)) = σ·χ(w)` (work in `Module.End` with `*`, not `∘ₗ`, so `Finset.mul_sum`/`smul_mul_assoc`/`mul_smul_comm` apply; `← Module.End.mul_eq_comp` converts the `linHom_apply` `∘ₗ` to `*`; `← Module.End.one_eq_id` lets `mul_one` clear the `id`). Then: **idempotency** = the `z = χ̃_V` case, scalar pinned to `1` by evaluating at `g=1` (`FDRep.char_orthonormal V V` + `if_pos ⟨Iso.refl V⟩` + `invOf_smul_eq_iff` give `∑_g χ(g)χ(g⁻¹) = |G|`); `χ̃_V ≠ 0` from `χ̃_V(1) = (dim V)²/|G| ≠ 0`. **Primitivity** = the standard minimal-idempotent argument in the centre `CommRing`: from `χ̃_V = a+b` (a,b nonzero idempotents), `ab = 0` (char 0 — `(2 : ↥(classFunctions G))` is a unit via `(two_ne_zero (α:=ℂ)).isUnit |>.map (algebraMap ℂ _)` after `rw [show (2:_) = algebraMap ℂ _ 2 by rw [map_ofNat]]`, then `IsUnit.mul_right_eq_zero`), so `a = χ̃_V·a = σ_a • χ̃_V` (absorption + `hea : χ̃_V·a = a` from `ab=0`), and `a` idempotent + nonzero forces `σ_a = 1` (cancel `s • χ̃_V = t • χ̃_V → s = t` by evaluating the coefficient at `1` — NO `NoZeroSMulDivisors` instance needed), giving `a = b = χ̃_V` and `χ̃_V = 2χ̃_V`, contradiction. Gotchas: write the FDRep carrier as bare `V` (auto-coerces; `(↑V : Type u)` FAILS — the `ℂ`-module carrier lives in `Type 0`, not `Type u`); to `rw [smul_mul_smul_comm]` into an idempotent first restate it as `have h' : a*a = a := ha` (the `IsIdempotentElem` `def` won't expose the `•*•` pattern); lift underlying↔subtype equations with `Subtype.ext` + `simp only [MulMemClass.coe_mul, SetLike.val_smul]`.
