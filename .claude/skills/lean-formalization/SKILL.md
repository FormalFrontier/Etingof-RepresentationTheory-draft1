...
5. **Referencing a def over `[Algebra k A] [Module A V]`: the *base ring* `k` is not inferred from a `(V := V)` named arg.** When a declaration is parameterized over a module-over-an-algebra bundle (e.g. `vertexProj i : Module.End k V` with `[Module k V] [Module (PathAlgebra k Q) V] [IsScalarTower k (PathAlgebra k Q) V]`), referring to it with a *named argument* `foo (V := V) …` leaves `k` (and often `Q`) as stuck metavars — `typeclass instance problem is stuck: Field ?m` / `IsScalarTower ?k (PathAlgebra ?k Q) V` — because instance search does **not** back-infer `k` from `V`'s module instance. Three fixes that worked first try in `Chapter2/Discussion_quiver_rep_bijection.lean` (#5149), in preference order: (a) **type-ascribe the whole term/family** rather than named-arg it: `(vertexProj i : Module.End k V)`, `(fun i : Q => (vertexProj i : Module.End k V))` for a bare `CompleteOrthogonalIdempotents`/`IsInternal` family, and `∑ i : Q, …` (pin the sum's index type, else `Fintype ?m` stalls); (b) on a `rfl` apply-lemma, ascribe **both** sides (`(vertexProj i : …) v = (trivialPath i : PathAlgebra k Q) • v`) — a bare `trivialPath i • v` RHS reintroduces an independent `?k`; (c) add a `rfl` `_eq` lemma (`vertexSpace_eq : vertexSpace i = LinearMap.range (vertexProj i) := rfl`) so `rw` can expose the `range`/underlying form (`rw [vertexSpace, …]` fails with "Invalid rewrite argument `?m`" when the def's args are unpinned).
