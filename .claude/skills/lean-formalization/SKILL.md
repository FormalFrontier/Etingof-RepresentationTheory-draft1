...
- **Nondegenerate alternating form ⟹ even `finrank` (symplectic even-dim, #5236).** Skip the symplectic-basis induction the issue suggested — the determinant argument is ~15 lines. Alternating (`∀ v, B v v = 0`) ⟹ skew-symmetric: expand `B (v+w) (v+w) = 0` with `simp only [map_add, LinearMap.add_apply, halt]` then `linear_combination`. The Gram matrix `M = LinearMap.BilinForm.toMatrix b B` (basis `b := Module.finBasis ℂ V`) then satisfies `M.transpose = -M`, so `M.det = det Mᵀ = det(-M) = (-1)ⁿ M.det` (`Matrix.det_transpose`, `Matrix.det_neg` — the latter gives `(-1)^(Fintype.card (Fin n))`, clear with `simp [Fintype.card_fin]`). Odd `n` ⟹ `M.det = -M.det` (`Odd.neg_one_pow`) ⟹ `M.det = 0` via `linear_combination (1/2 : ℂ) * h1`, contradicting `det ≠ 0`. **Two gotchas cost ~4 build cycles:** (1) `LinearMap.BilinForm.Nondegenerate B` does **NOT** defeq-unfold to `∀ m, (∀ n, B m n = 0) → m = 0` during elaboration — `:= hnondeg`, a `fun m hm => …` lambda, `show`, and `unfold` all fail with a type mismatch. Convert via `rw [LinearMap.BilinForm.nondegenerate_iff_ker_eq_bot, LinearMap.ker_eq_bot']; intro m hm; exact hnondeg m (fun w => by rw [hm]; simp)`. Then `(LinearMap.BilinForm.nondegenerate_iff_det_ne_zero b).mp` gives `M.det ≠ 0`. (2) The `ᵀ` transpose notation needs `open scoped Matrix`; use `M.transpose` to avoid the import. Note `B.Nondegenerate` dot-notation resolves against `LinearMap`, not `BilinForm` — always write `LinearMap.BilinForm.Nondegenerate B` in full. Proof landed in `Chapter5/FrobeniusSchurRealType.lean` as `even_finrank_of_nondegenerate_alternating` (closes L3 `even_finrank_of_isQuaternionicType`).
