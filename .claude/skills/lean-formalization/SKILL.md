...
8. **A theorem can be unprovable because its *type class is too weak* — audit whether the stated hypotheses actually entail the goal, and look for a model of the class where the goal fails.** When a "real theorem with placeholder proof" resists, check that the ambient instances logically force the conclusion; if a concrete category/structure satisfies all the class fields yet violates the statement, the lemma is unprovable *as typed* and needs a stronger hypothesis (escalate as "Definition seems wrong", don't grind). Example (#5320, `Chapter9/KrullSchmidt/Length.lean`): `clength_eq_zero_iff`/`clength_additive` are stated over `Etingof.IsFiniteAbelianCategory`, but that class is only `Abelian` + `EnoughProjectives` + finitely-many-simples — it does **not** require finite length. The category of *all* modules over `k[x]/(x²)` satisfies every field (one simple `k`, enough projectives) yet has infinite-length objects, for which `clength = (Order.height ⊤).toNat = 0` (since `toNat ⊤ = 0`) while the object is nonzero — so `clength_eq_zero_iff` is false there. The finite-length condition lives in a *separate* class (`HasFiniteLength` / `IsFiniteAbelianCategoryOverField`); the fix is to route it in (#5324), not to attempt the proof. **`Order.height … |>.toNat` silently collapses the infinite case to `0`, so any `(height ⊤).toNat`-based length is only meaningful once finiteness is separately supplied.** Sibling lemmas often *reduce* to the genuine crux cheaply even when the crux is blocked: `clength_pos_of_not_isZero` is just the contrapositive of `clength_eq_zero_iff`, and `clength_biprod` is `clength_additive` on the split biproduct SES (`ShortComplex.Splitting` with `r := biprod.fst, s := biprod.inr, id := biprod.total`, then `Splitting.shortExact`) — so 2 of the 4 sorries discharge by reduction while the 2 cruxes stay blocked.

This saved 2+ sessions in Waves 47-49 by catching false statements early, an entire D̃₄ proof attempt (#4566), a Ch5 Wall 3 R2.b.i attempt against a false pointwise-vanishing residual lemma (#2769 → #4584), and a research-level Cauchy proof attempt against a false multiplicity-one character identity (#4944), plus the Ch9 `clength` cruxes (#5320) being unprovable over the too-weak `IsFiniteAbelianCategory` class.
