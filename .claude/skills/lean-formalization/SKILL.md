...
1. **Check Known Dead-Ends.** Scan the "Known Dead-Ends" section below. If your proof requires any of these patterns, sorry it immediately and move on:
   ...
   - Non-commutative `Module.Flat` — likewise commutative-base only. `Module.Flat A P` for a noncommutative algebra `A` does **not** typecheck (`failed to synthesize CommSemiring A`; `class Flat` lives under `variable [CommSemiring R]` in `Mathlib/RingTheory/Flat/Basic.lean`). Mathlib has no left-module flatness predicate over a noncommutative ring; `iff_characterModule_injective` (Lambek) only works because its base is `ℤ`. Any "P flat as a left A-module" statement (e.g. Example 8.1.7 / #5475) must hand-roll the flatness predicate (`∀` injective right-`A`-linear `f`, `f ⊗ id_P` injective) AND first build the balanced tensor `X ⊗_A P` above. Verify such API typechecks *before* committing to a flat/Lambek proof strategy.
