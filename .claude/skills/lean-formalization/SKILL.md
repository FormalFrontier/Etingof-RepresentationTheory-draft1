...
- **Phase B of the `ℂ³₊`/`ℂ³₋` split landed (#5458 deliverables 1+3 → PR #5463; remainder #5462).** Two reusable lessons. (a) **Build the eigenspace subrep from an *ambient* operator, not the subtype operator.** `lam2` lives on `range asym ⊆ W4 ⊗ W4`; its `zEnd` is a `Module.End ↥lam2Sub.toSubmodule`. Packaging `Module.End.eigenspace zEnd μ` (a submodule of the *subtype* `↥lam2Sub.toSubmodule`) as `FDRep.of …toRepresentation` **fails**: `FDRep.of` needs `[AddCommGroup V]` but the subtype-of-subtype carrier hits an `AddCommGroup`/`AddCommMonoid` instance diamond that does not resolve even at `synthInstance.maxHeartbeats 400000` (symptom: first a synth *timeout*, then an "Application type mismatch … `Submodule.addCommMonoid` vs `AddCommGroup.toAddCommMonoid`"). **Fix:** define the *ambient* `Zamb : Module.End ℂ (W4 ⊗[ℂ] W4) := Σ_g (rhoV.tprod rhoV)(g·r·g⁻¹)` and make the carrier `lam2Sub.toSubmodule ⊓ Module.End.eigenspace Zamb μ` — a plain submodule of `W4 ⊗ W4`, one nesting level deep (exactly `lam2Sub`'s shape), which `FDRep.of` accepts. Invariance: `Submodule.mem_inf` + `lam2Sub.apply_mem_toSubmodule` (for `range asym`) and `Module.End.mapsTo_genEigenspace_of_comm (Zamb_comm h).symm μ 1` (for the eigenspace; `eigenspace = genEigenspace μ 1` is an `abbrev`). (b) **Centrality of a full-group class-sum is a one-liner.** `Commute (ρ h) (Σ_g ρ(g·r·g⁻¹))` via `show … ; rw [Finset.mul_sum, Finset.sum_mul, ← Equiv.sum_comp (Equiv.mulLeft h⁻¹) …]; refine Finset.sum_congr rfl …; simp only [Equiv.coe_mulLeft]; rw [← map_mul, ← map_mul]; congr 1; group`. **Normalization caveat:** #5458 sums over **all of `A₅`** (`z = Σ_{g∈G} ρ(g·r·g⁻¹) = 5·(class-sum)`, centralizer order 5), so eigenvalues are `μ± = 10±10√5 = 20φ/20φ'`, **min poly `X²−20X−400`** — not the `X²−4X−16` of the bare class-sum above. Pick one normalization and keep it; #5462's recipe uses the full-group `z`.
