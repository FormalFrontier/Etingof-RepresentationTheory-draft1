...
- **Trace-power-sum `trace(fᵏ) = ∑_μ (dim Eμ)·μᵏ` for a finite-order operator, and the cyclotomic-character identity `σ(χ_V(g)) = χ_V(gʲ)` (#5772, `Chapter5/Remark5_2_8.lean`).** Mathlib has `Matrix.trace_eq_sum_roots_charpoly` (trace = *sum* of eigenvalues) but **NOT** the power-sum-with-multiplicity `trace(fᵏ) = ∑ λᵢᵏ`. Do not chase "roots of charpoly of `fᵏ`" — go through the **eigenspace decomposition**, which a finite-order `f` (`f^N = 1`, `N > 0`) always admits over ℂ: `f` is semisimple because `X^N - 1` is squarefree (`Polynomial.X_pow_sub_one_separable_iff.mpr` (n≠0 in ℂ) `|>.squarefree`) and annihilates `f` (`aeval f (X^N-1) = 0` by `simp [map_sub, hf]`), so `Module.End.isSemisimple_of_squarefree_aeval_eq_zero`. Then `(DirectSum.isInternal_submodule_iff_iSupIndep_and_iSup_eq_top _).mpr ⟨f.eigenspaces_iSupIndep, hss.iSup_eigenspace_eq_top⟩` gives `IsInternal (Module.End.eigenspace f)`, and `LinearMap.trace_eq_sum_trace_restrict'` (the *primed* one — takes `{μ | eigenspace ≠ ⊥}.Finite`, no `Fintype ℂ` needed; finiteness is `Module.End.finite_hasEigenvalue f`) reduces `trace(fᵏ)` to a sum over eigenvalues. On `Eμ`, `(fᵏ).restrict = μᵏ • id` (from `pow_apply_of_mem_eigenspace : x ∈ eigenspace f μ → fᵏ x = μᵏ • x`, an easy `pow_succ'` induction), so each term is `μᵏ • trace(id) = μᵏ * finrank` (`LinearMap.trace_id`). To then get `σ(χ_V(g)) = χ_V(gʲ)` for a ring hom `σ : ℂ →+* ℂ` powering roots of unity: apply the trace-power-sum at `k=1` and `k=j` with `f = V.ρ g` (`f^{|G|}=1` via `map_pow` + `pow_card_eq_one`; `V.character g = LinearMap.trace ℂ V (V.ρ g)` by `rfl`), then `map_sum`/`map_mul`/`map_natCast` + `σ μ = μʲ` (each eigenvalue is an `|G|`-th root of unity, `eigenvalue_pow_eq_one`). Two gotchas: (a) `rw [pow_one] at e1` rewrites only the *first* unified instance (`f^1`), leaving `μ^1` — use `simp only [pow_one]`; (b) when `f : V →ₗ[ℂ] V` (e.g. `V.ρ g`, not typed `Module.End`), dot notation `f.eigenspace` resolves to `LinearMap.eigenspace` and fails — write `Module.End.eigenspace f μ` / `Module.End.finite_hasEigenvalue f`. Combined with a `prod_bij` reindex (`g ↦ gʲ`, `j` coprime to `|G|`) this gives `σ(β) = β` for `β = ∏_{g≠1} χ_V(g)·χ_V(g⁻¹)` (the honest polynomial form of `∏|χ_V(g)|²`, since `σ` is a ring hom but conjugation is not σ-equivariant — use `χ_V(g⁻¹)`, not `conj χ_V(g)`).
