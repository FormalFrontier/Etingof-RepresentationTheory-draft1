...
- **`rw [FDRep.of_ρ']` fails when the FDRep is a *def alias* of `FDRep.of …`** (e.g. `SchurModule k n lz := FDRep.of (schurModuleRep …)`, `AlgIrrepGLRep`): the pattern `(FDRep.of ?ρ).ρ` is not syntactically present, so `rw` reports "did not find pattern". Unfold the alias first: `simp only [SchurModule, FDRep.of_ρ']`. To bridge `glWeightSpaceℤ … (schurModuleRep …)` and `glWeightSpace … (SchurModule …)` (different but defeq carriers — `SchurModuleSubmodule` vs `(SchurModule …).V`), rewrite the `glWeightSpace`-side lemma at a hypothesis, `simp only [SchurModule, FDRep.of_ρ'] at h`, then `rw [← h]` and close with a bare **`rfl`** (the two `finrank`s are defeq but `rw`'s reducible-transparency post-rfl won't fire; the standalone `rfl` tactic, at default transparency, will). Also: lemmas like `glWeightSpace_schurModule_iSup_eq_top` are stated with `k : Type` (not `Type*`) — a `Type*` caller triggers a universe mismatch + `isDefEq` timeout, so match `k : Type` in the helper signatures. (#5561)
