...
*
  * Representatives
  * Number of elements in a conjugacy class
  * Number of classes
*
  * Scalar $`\begin{pmatrix} x & 0 \\ 0 & x \end{pmatrix}`
  * 1 (this is a central element)
  * $`q - 1` (one for every nonzero $`x`)
*
  * Parabolic $`\begin{pmatrix} x & 1 \\ 0 & x \end{pmatrix}`
  * $`q^2 - 1` (elements that commute with this one are of the form $`\begin{pmatrix} t & u \\ 0 & t \end{pmatrix}`, $`t \neq 0`)
  * $`q - 1` (one for every nonzero $`x`)
*
  * Hyperbolic $`\begin{pmatrix} x & 0 \\ 0 & y \end{pmatrix}`, $`y \neq x`
  * $`q^2 + q` (elements that commute with this one are of the form $`\begin{pmatrix} t & 0 \\ 0 & u \end{pmatrix}`, $`t, u \neq 0`)
  * $`\frac{1}{2}(q - 1)(q - 2)` ($`x, y \neq 0` and $`x \neq y`)
*
  * Elliptic $`\begin{pmatrix} x & \varepsilon y \\ y & x \end{pmatrix}`, $`x \in \mathbb{F}_q`, $`y \in \mathbb{F}_q^\times`, $`\varepsilon \in \mathbb{F}_q \setminus \mathbb{F}_q^2` (characteristic polynomial over $`\mathbb{F}_q` is irreducible)
  * $`q^2 - q` (the reason will be described below)
  * $`\frac{1}{2}q(q - 1)` (matrices with $`y` and $`-y` are conjugate)
:::
