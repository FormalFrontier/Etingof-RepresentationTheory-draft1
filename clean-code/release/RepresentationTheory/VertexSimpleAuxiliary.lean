...
theorem auxiliaryMap_auxiliaryElement (i : Q) :
  ...
  rw [auxiliaryVertexIdempotent, ofPath, auxiliaryRingHom_apply, auxiliaryLinearMap_single,
    if_pos rfl]
...
theorem auxiliaryMap_algebraMap (i : Q) (c : k) :
    auxiliaryRingHom i (algebraMap k (PathAlgebra k Q) c) = c := by
...
theorem auxiliaryElement_ne_zero (i : Q) :
    (auxiliaryVertexIdempotent i : PathAlgebra k Q) ≠ 0 := by
  intro h
...
theorem span_secondaryAuxiliaryElement_eq_top (i : Q) :
    Submodule.span (PathAlgebra k Q) {auxiliaryVertexElement k Q i} = ⊤ := by
...
theorem auxiliary_isSimpleModule (i : Q) :
    IsSimpleModule (PathAlgebra k Q) (Auxiliary k Q i) := by
...
noncomputable def toSecondaryAuxiliaryLinearMap (i : Q) :
    PathAlgebra k Q →ₗ[PathAlgebra k Q] AuxiliaryVertexSpace k Q i :=
  LinearMap.codRestrict (Submodule.span (PathAlgebra k Q) {auxiliaryVertexIdempotent (k := k) i})
    (LinearMap.mulRight (PathAlgebra k Q) (auxiliaryVertexIdempotent i))
...
@[simp] theorem toSecondaryAuxiliaryLinearMap_apply (i : Q) (a : PathAlgebra k Q) :
    ((toSecondaryAuxiliaryLinearMap i a : AuxiliaryVertexSpace k Q i) : PathAlgebra k Q) =
...
instance secondaryAuxiliary_projective (i : Q) :
    Module.Projective (PathAlgebra k Q) (AuxiliaryVertexSpace k Q i) :=
  Module.Projective.of_split
    (Submodule.span (PathAlgebra k Q) {auxiliaryVertexIdempotent (k := k) i}).subtype
...
theorem finrank_end_secondaryAuxiliary_eq_one
    (hacyclic : ∀ (v : Q) (p : Quiver.Path v v), p = Quiver.Path.nil) (i : Q) :
    Module.finrank k
        (AuxiliaryVertexSpace k Q i →ₗ[PathAlgebra k Q] AuxiliaryVertexSpace k Q i) = 1 := by
...
theorem linearMap_id_ne_zero (i : Q) :
    (LinearMap.id : AuxiliaryVertexSpace k Q i →ₗ[PathAlgebra k Q]
...
theorem auxiliaryPredicate_of_no_nontrivial_loops
    ...
    RepresentationTheory.LinearAlgebra.ModuleDecompositions.AuxiliaryDecompositionPredicate
      (PathAlgebra k Q) (AuxiliaryVertexSpace k Q i) := by
  refine ⟨inferInstance, fun W₁ W₂ hc => ?_⟩
  set P := W₁.projection W₂ hc with hPdef
  obtain ⟨c, hcP⟩ :=
    (finrank_eq_one_iff_of_nonzero' (LinearMap.id :
        AuxiliaryVertexSpace k Q i →ₗ[PathAlgebra k Q] AuxiliaryVertexSpace k Q i)
  ...
  have hcc : c * c = c := by
    have h0 : (c * c - c) • (LinearMap.id :
      ...
      exact (sub_smul (c * c) c (LinearMap.id :
        AuxiliaryVertexSpace k Q i →ₗ[PathAlgebra k Q] AuxiliaryVertexSpace k Q i)).trans
          (sub_eq_zero.mpr hc2)
...
noncomputable def secondaryToAuxiliaryLinearMap (i : Q) :
    AuxiliaryVertexSpace k Q i →ₗ[PathAlgebra k Q] Auxiliary k Q i where
  toFun y := ULift.up (auxiliaryRingHom i (y : PathAlgebra k Q))
  map_add' y z := by apply ULift.ext; exact map_add (auxiliaryRingHom i) _ _
  map_smul' a y := by
    apply ULift.ext
    rw [auxiliary_smul_down]
    exact map_mul (auxiliaryRingHom i) a (y : PathAlgebra k Q)
...
@[simp] theorem secondaryToAuxiliaryLinearMap_down_apply
    ...
    (secondaryToAuxiliaryLinearMap i y).down = auxiliaryRingHom i (y : PathAlgebra k Q) := rfl
...
theorem secondaryToAuxiliaryLinearMap_surjective (i : Q) :
  ...
  intro w
  refine ⟨(algebraMap k (PathAlgebra k Q) w.down) • auxiliaryVertexElement k Q i, ?_⟩
  apply ULift.ext
...
theorem auxiliaryElement_mul_eq_self_of_mem_span
    (hacyclic : ∀ (v : Q) (p : Quiver.Path v v), p = Quiver.Path.nil) {i : Q}
    {n : PathAlgebra k Q}
    (hn : n ∈ Submodule.span (PathAlgebra k Q) {auxiliaryVertexIdempotent (k := k) i})
  ...
  refine ext fun x => ?_
  by_cases hx : x = (⟨i, i, Quiver.Path.nil⟩ : Quiver.BundledPath Q)
  · subst hx
    rw [coeff_vertexIdempotent_mul, if_pos rfl, auxiliaryVertexIdempotent, ofPath,
  ...
  · have hcut : auxiliaryVertexIdempotent i * (a * auxiliaryVertexIdempotent i) ∈
    ...
    rw [coeff_eq_zero_of_mem_pathComponent hcut ?_, auxiliaryVertexIdempotent, ofPath,
...
theorem eq_top_of_sup_ker_secondaryToAuxiliary_eq_top
    (hacyclic : ∀ (v : Q) (p : Quiver.Path v v), p = Quiver.Path.nil) (i : Q)
    (N : Submodule (PathAlgebra k Q) (AuxiliaryVertexSpace k Q i))
  ...
  have haug : auxiliaryRingHom i (n : PathAlgebra k Q) = 1 := by
  ...
  have hgen : (auxiliaryVertexIdempotent i : PathAlgebra k Q) • n =
...
noncomputable def auxiliaryStructureOfNoNontrivialLoops
    ...
    RepresentationTheory.RingModuleAuxiliary.Auxiliary
      (PathAlgebra k Q) (Auxiliary k Q i) where
...
theorem sum_auxiliaryElement_eq_one :
    (∑ i : Q, (auxiliaryVertexIdempotent i : PathAlgebra k Q)) = 1 :=
...
theorem not_linearEquiv_auxiliary_of_ne {i j : Q} (h : i ≠ j)
    (e : Auxiliary k Q i ≃ₗ[PathAlgebra k Q] Auxiliary k Q j) : False := by
  have h1 : (auxiliaryVertexIdempotent i : PathAlgebra k Q) •
      auxiliary (k := k) (Q := Q) i = auxiliary i := by
    apply ULift.ext
    rw [auxiliary_smul_down, auxiliaryMap_auxiliaryElement, one_mul]
  have h2 : (auxiliaryVertexIdempotent i : PathAlgebra k Q) •
...
theorem exists_linearEquiv_auxiliary
    ...
    ∃ i : Q, Nonempty (M ≃ₗ[PathAlgebra k Q] Auxiliary k Q i) := by
  haveI : Nontrivial M := IsSimpleModule.nontrivial (PathAlgebra k Q) M
  obtain ⟨m, hm⟩ := exists_ne (0 : M)
  obtain ⟨i, hi⟩ : ∃ i : Q, (auxiliaryVertexIdempotent (k := k) i : PathAlgebra k Q) • m ≠ 0 := by
    by_contra hcon
    refine hm ?_
    have h0 : (∑ i : Q, (auxiliaryVertexIdempotent (k := k) i : PathAlgebra k Q)) • m = 0 := by
      rw [Finset.sum_smul]
      refine Finset.sum_eq_zero fun j _ => ?_
      by_contra hne
      exact hcon ⟨j, hne⟩
    rwa [sum_auxiliaryElement_eq_one, one_smul] at h0
  set f : AuxiliaryVertexSpace k Q i →ₗ[PathAlgebra k Q] M :=
  ...
  have hker : LinearMap.ker f ≤ LinearMap.ker (secondaryToAuxiliaryLinearMap i) := by
    ...
    have hspan : Submodule.span (PathAlgebra k Q) {secondaryToAuxiliaryLinearMap i y} = ⊤ := by
      rcases IsSimpleOrder.eq_bot_or_eq_top
        (Submodule.span (PathAlgebra k Q) {secondaryToAuxiliaryLinearMap i y}) with h | h
      · refine absurd ?_ hyp'
    ...
    have htop : LinearMap.ker f ⊔ LinearMap.ker (secondaryToAuxiliaryLinearMap i) = ⊤ := by
      ...
      have hz : secondaryToAuxiliaryLinearMap i z ∈
          Submodule.span (PathAlgebra k Q) {secondaryToAuxiliaryLinearMap i y} := by
...
open RepresentationTheory.Quiver.PathAlgebra.Quiver.PathAlgebra
...
theorem auxiliaryResults_of_no_nontrivial_loops
    ...
    [∀ i j : Q, Finite (i ⟶ j)] :
    (∀ i : Q, IsSimpleModule
      (_root_.RepresentationTheory.Quiver.PathAlgebra.Quiver.PathAlgebra k Q) (Auxiliary k Q i)) ∧
      (∀ i j : Q, Nonempty
        (Auxiliary k Q i ≃ₗ[_root_.RepresentationTheory.Quiver.PathAlgebra.Quiver.PathAlgebra k Q]
          Auxiliary k Q j) → i = j) ∧
      (∀ (M : Type w) [AddCommGroup M]
          [Module (_root_.RepresentationTheory.Quiver.PathAlgebra.Quiver.PathAlgebra k Q) M]
          [IsSimpleModule (_root_.RepresentationTheory.Quiver.PathAlgebra.Quiver.PathAlgebra k Q) M],
        ∃ i : Q, Nonempty
          (M ≃ₗ[_root_.RepresentationTheory.Quiver.PathAlgebra.Quiver.PathAlgebra k Q]
            Auxiliary k Q i)) ∧
