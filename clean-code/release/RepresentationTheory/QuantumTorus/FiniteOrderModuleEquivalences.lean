...
namespace RepresentationTheory.QuantumTorus.FiniteOrderModuleEquivalences
...
theorem auxiliaryIndexedUnit_conj (X Y E : (Module.End k V)ˣ) (i j : ℤ) :
    monomialEnd (E * X * E⁻¹) (E * Y * E⁻¹) i j =
      ↑E * monomialEnd X Y i j * ↑E⁻¹ := by
  ...
  simpa [monomialEnd, mul_assoc] using hval

/-- If a unit intertwines each of two pairs of generators satisfying the same quantum commutation relation, then it conjugates the associated representations on every algebra element. -/
theorem representation_apply_eq_conj_of_intertwines_generators
    (X Y X' Y' E : (Module.End k V)ˣ)
    (hrel : (↑Y : Module.End k V) * ↑X = (q : k) • (↑X * ↑Y))
    (hrel' : (↑Y' : Module.End k V) * ↑X' = (q : k) • (↑X' * ↑Y'))
    (hX : E * X = X' * E) (hY : E * Y = Y' * E)
    (a : twistedLatticeShiftSubalgebra k q) :
    representationOfQCommute q X' Y' hrel' a =
      ↑E * representationOfQCommute q X Y hrel a * ↑E⁻¹ := by
  have hX' : X' = E * X * E⁻¹ := by rw [hX, mul_inv_cancel_right]
  have hY' : Y' = E * Y * E⁻¹ := by rw [hY, mul_inv_cancel_right]
  subst hX'
  subst hY'
  have key :
      (representationOfQCommute q (E * X * E⁻¹) (E * Y * E⁻¹) hrel').toLinearMap =
        (LinearMap.mulRight k (↑E⁻¹ : Module.End k V)).comp
          ((LinearMap.mulLeft k (↑E : Module.End k V)).comp
            (representationOfQCommute q X Y hrel).toLinearMap) := by
    refine (monomialBasis q).ext fun p => ?_
    simp only [monomialBasis_apply, AlgHom.toLinearMap_apply, LinearMap.coe_comp,
      Function.comp_apply, LinearMap.mulRight_apply, LinearMap.mulLeft_apply,
      representationOfQCommute_monomial]
    exact auxiliaryIndexedUnit_conj X Y E p.1 p.2
  exact DFunLike.congr_fun key a

end RepresentationTheory.QuantumTorus.FiniteOrderModuleEquivalences
...
open RepresentationTheory.Algebra.Module.TwistedLatticeShifts
  ...
  RepresentationTheory.QuantumTorus.FiniteOrderModules Module

/-! ### Powers of the parameter on `Fin N` -/
...
/-! ### Powers of the cyclic shift intertwine the diagonal actions -/

section Intertwine

variable (q α β β' : ℂˣ) (N : ℕ) [NeZero N]

/-- Shifting the index of a diagonal weight coefficient matches changing the weight parameter by the corresponding quantum-parameter power. -/
theorem diagonalWeightCoeff_sub_nsmul_eq (hqorder : orderOf q = N) (m : ℕ)
    (hβ : (β : ℂ) = (β' : ℂ) * (q : ℂ) ^ m) (k : Fin N) :
    twoUnitIndexedScalar q β N (k - m • (1 : Fin N)) =
      twoUnitIndexedScalar q β' N k := by
...
/-- A power of the cyclic shift intertwines diagonal weight endomorphisms whose weight parameters differ by the corresponding power of the quantum parameter. -/
theorem cyclicShiftEnd_pow_mul_diagonalWeightEnd (hqorder : orderOf q = N) (m : ℕ)
    (hβ : (β : ℂ) = (β' : ℂ) * (q : ℂ) ^ m) :
    (cyclicShiftEnd α N ^ m) * (diagonalWeightEnd q β N) =
      (diagonalWeightEnd q β' N) * (cyclicShiftEnd α N ^ m) := by
  refine LinearMap.ext fun f => ?_
  funext k
  simp only [Module.End.mul_apply, diagonalWeightEnd, LinearMap.coe_mk, AddHom.coe_mk,
    smul_eq_mul]
  rw [cyclicShiftEnd_pow_apply, cyclicShiftEnd_pow_apply,
    ← diagonalWeightCoeff_sub_nsmul_eq q β β' N hqorder m hβ k]
  simp only [smul_eq_mul]
  ring

/-- A power of the cyclic shift unit intertwines diagonal weight units whose weight parameters differ by the corresponding power of the quantum parameter. -/
theorem cyclicShiftUnit_pow_mul_diagonalWeightUnit (hqorder : orderOf q = N) (m : ℕ)
    (hβ : (β : ℂ) = (β' : ℂ) * (q : ℂ) ^ m) :
    (cyclicShiftUnit α N ^ m) * (diagonalWeightUnit q β N) =
      (diagonalWeightUnit q β' N) * (cyclicShiftUnit α N ^ m) := by
  refine Units.ext ?_
  simp only [Units.val_mul, Units.val_pow_eq_pow_val, cyclicShiftUnit_val,
    diagonalWeightUnit_val]
...
theorem cyclicShiftUnit_pow_mul_cyclicShiftUnit (m : ℕ) :
    (cyclicShiftUnit α N ^ m) * (cyclicShiftUnit α N) =
      (cyclicShiftUnit α N) * (cyclicShiftUnit α N ^ m) :=
  (Commute.refl (cyclicShiftUnit α N)).pow_left m

end Intertwine

/-! ### The parameterized carrier -/
...
instance threeUnitParameterAddCommGroup : AddCommGroup (ThreeUnitParameterType q α β) :=
  inferInstanceAs (AddCommGroup (Fin (orderOf q) → ℂ))

/-- The complex module structure on the three-unit-parameter type. -/
instance threeUnitParameterComplexModule : Module ℂ (ThreeUnitParameterType q α β) :=
  inferInstanceAs (Module ℂ (Fin (orderOf q) → ℂ))
...
instance threeUnitParameter_nontrivial : Nontrivial (ThreeUnitParameterType q α β) :=
  inferInstanceAs (Nontrivial (Fin (orderOf q) → ℂ))

/-- The displayed algebra module structure on the type parameterized by three complex units. -/
noncomputable instance threeUnitParameterModule :
    Module (twistedLatticeShiftSubalgebra ℂ q) (ThreeUnitParameterType q α β) :=
...
instance threeUnitParameterIsScalarTower :
    IsScalarTower ℂ (twistedLatticeShiftSubalgebra ℂ q) (ThreeUnitParameterType q α β) :=
...
theorem threeUnitParameter_smul_eq_representation_apply
    (a : twistedLatticeShiftSubalgebra ℂ q) (f : ThreeUnitParameterType q α β) :
    a • f = representationOfQCommute q (cyclicShiftUnit α (orderOf q))
      (diagonalWeightUnit q β (orderOf q))
...
theorem finrank_threeUnitParameterType :
    Module.finrank ℂ (ThreeUnitParameterType q α β) = orderOf q :=
...
theorem firstGenerator_smul_threeUnitParameterType (f : ThreeUnitParameterType q α β) :
    (monomial q (1, 0)) • f = cyclicShiftEnd α (orderOf q) f :=
...
theorem secondGenerator_smul_threeUnitParameterType (f : ThreeUnitParameterType q α β) :
    (monomial q (0, 1)) • f = diagonalWeightEnd q β (orderOf q) f :=
  secondGenerator_smul q α β (orderOf q) rfl f

/-- The distinguished element with first exponent equal to the quantum-parameter order acts by the shift parameter. -/
theorem firstGenerator_orderExponent_smul_threeUnitParameterType
    (f : ThreeUnitParameterType q α β) :
    (monomial q ((orderOf q : ℤ), 0)) • f = (α : ℂ) • f :=
  firstGenerator_cardExponent_smul q α β (orderOf q) rfl f

/-- The distinguished element with second exponent equal to the quantum-parameter order acts by the corresponding power of the weight parameter. -/
theorem secondGenerator_orderExponent_smul_threeUnitParameterType
    (f : ThreeUnitParameterType q α β) :
    (monomial q (0, (orderOf q : ℤ))) • f = ((β : ℂ) ^ orderOf q) • f :=
...
noncomputable def auxiliaryEndomorphismUnit (m : ℕ) :
    (Module.End ℂ (Fin (orderOf q) → ℂ))ˣ := cyclicShiftUnit α (orderOf q) ^ m
...
theorem auxiliaryEndomorphismUnit_inv_apply_apply (m : ℕ) (g : Fin (orderOf q) → ℂ) :
    (((auxiliaryEndomorphismUnit q α m)⁻¹ :
        (Module.End ℂ (Fin (orderOf q) → ℂ))ˣ) : Module.End ℂ (Fin (orderOf q) → ℂ))
      (((auxiliaryEndomorphismUnit q α m :
        (Module.End ℂ (Fin (orderOf q) → ℂ))ˣ) : Module.End ℂ (Fin (orderOf q) → ℂ)) g) = g := by
...
noncomputable def moduleLinearEquivOfUnitPowerRelation
    (m : ℕ) (hβ : (β : ℂ) = (β' : ℂ) * (q : ℂ) ^ m) :
    ThreeUnitParameterType q α β ≃ₗ[twistedLatticeShiftSubalgebra ℂ q]
      ThreeUnitParameterType q α β' where
  toFun f := ((auxiliaryEndomorphismUnit q α m :
    (Module.End ℂ (Fin (orderOf q) → ℂ))ˣ) : Module.End ℂ (Fin (orderOf q) → ℂ)) f
  invFun f := (((auxiliaryEndomorphismUnit q α m)⁻¹ :
    (Module.End ℂ (Fin (orderOf q) → ℂ))ˣ) : Module.End ℂ (Fin (orderOf q) → ℂ)) f
  map_add' f g := map_add _ f g
  map_smul' a f := by
    have hconj := representation_apply_eq_conj_of_intertwines_generators q
      (cyclicShiftUnit α (orderOf q)) (diagonalWeightUnit q β (orderOf q))
      (cyclicShiftUnit α (orderOf q)) (diagonalWeightUnit q β' (orderOf q))
      (auxiliaryEndomorphismUnit q α m)
      (diagonalWeightUnit_val_mul_cyclicShiftUnit_val q α β (orderOf q) rfl)
      (diagonalWeightUnit_val_mul_cyclicShiftUnit_val q α β' (orderOf q) rfl)
    ...
    change ((auxiliaryEndomorphismUnit q α m :
        (Module.End ℂ (Fin (orderOf q) → ℂ))ˣ) : Module.End ℂ (Fin (orderOf q) → ℂ))
        (representationOfQCommute q (cyclicShiftUnit α (orderOf q))
          (diagonalWeightUnit q β (orderOf q))
...
theorem parameters_eq_of_moduleLinearEquiv
    (e : ThreeUnitParameterType q α β ≃ₗ[twistedLatticeShiftSubalgebra ℂ q]
  ...
  constructor
  · have h1 : e ((monomial q ((orderOf q : ℤ), 0)) • v) =
        (monomial q ((orderOf q : ℤ), 0)) • e v := e.map_smul _ _
    rw [firstGenerator_orderExponent_smul_threeUnitParameterType,
      firstGenerator_orderExponent_smul_threeUnitParameterType,
      LinearMapClass.map_smul_of_tower e] at h1
    have h2 : ((α : ℂ) - (α' : ℂ)) • e v = 0 := by rw [sub_smul, h1, sub_self]
    rcases smul_eq_zero.mp h2 with h | h
    · exact Units.ext (sub_eq_zero.mp h)
    · exact absurd h hev
  · have h1 : e ((monomial q (0, (orderOf q : ℤ))) • v) =
        (monomial q (0, (orderOf q : ℤ))) • e v := e.map_smul _ _
    rw [secondGenerator_orderExponent_smul_threeUnitParameterType,
      secondGenerator_orderExponent_smul_threeUnitParameterType,
      LinearMapClass.map_smul_of_tower e] at h1
...
attribute [nolint unusedArguments]
  RepresentationTheory.QuantumTorus.FiniteOrderModuleEquivalences.ThreeUnitParameterType
