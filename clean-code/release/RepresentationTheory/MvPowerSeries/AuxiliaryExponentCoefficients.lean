...
namespace RepresentationTheory.MvPowerSeries.AuxiliaryExponentCoefficients

variable (N : ℕ) (k : Type*) [Field k]

/-- Associates an auxiliary finitely supported exponent index to a natural-valued function on finite indices. -/
def auxiliaryExponentIndex (α : Fin N → ℕ) : RepresentationTheory.LinearAlgebra.AuxiliaryPowerSeriesMatrix.AuxiliaryIndex N →₀ ℕ :=
  Finsupp.equivFunOnFinite.symm (Sum.elim α α)

/-- The auxiliary exponent index has value `α i` at `Sum.inl i`. -/
@[simp]
theorem auxiliaryExponentIndex_apply_inl (α : Fin N → ℕ) (i : Fin N) :
    auxiliaryExponentIndex N α (Sum.inl i) = α i := by
  simp [auxiliaryExponentIndex, Finsupp.equivFunOnFinite]

/-- The auxiliary exponent index has value `α j` at `Sum.inr j`. -/
@[simp]
theorem auxiliaryExponentIndex_apply_inr (α : Fin N → ℕ) (j : Fin N) :
    auxiliaryExponentIndex N α (Sum.inr j) = α j := by
  simp [auxiliaryExponentIndex, Finsupp.equivFunOnFinite]

private theorem prod_mul_prod_invOfUnit_eq_one (σ : Equiv.Perm (Fin N)) :
    (∏ j : Fin N,
      (1 - MvPowerSeries.X (Sum.inl j) * MvPowerSeries.X (Sum.inr (σ j)) :
        MvPowerSeries (RepresentationTheory.LinearAlgebra.AuxiliaryPowerSeriesMatrix.AuxiliaryIndex N) k)) *
    (∏ j : Fin N,
...
private theorem isUnit_prod_one_sub (σ : Equiv.Perm (Fin N)) :
    IsUnit (∏ j : Fin N,
      (1 - MvPowerSeries.X (Sum.inl j) * MvPowerSeries.X (Sum.inr (σ j)) :
        MvPowerSeries (RepresentationTheory.LinearAlgebra.AuxiliaryPowerSeriesMatrix.AuxiliaryIndex N) k)) := by
...
private theorem partialTarget_coeff (σ : Equiv.Perm (Fin N)) (S : Finset (Fin N))
    (d : RepresentationTheory.LinearAlgebra.AuxiliaryPowerSeriesMatrix.AuxiliaryIndex N →₀ ℕ) :
    MvPowerSeries.coeff (R := k) d (partialTarget N k σ S) =
    if (∀ j ∈ S, d (Sum.inl j) = d (Sum.inr (σ j))) ∧
       (∀ j : Fin N, j ∉ S → d (Sum.inl j) = 0 ∧ d (Sum.inr (σ j)) = 0)
    then 1 else 0 := rfl

private abbrev xyMon (σ : Equiv.Perm (Fin N)) (j : Fin N) : RepresentationTheory.LinearAlgebra.AuxiliaryPowerSeriesMatrix.AuxiliaryIndex N →₀ ℕ :=
...
private theorem coeff_one_sub_XmulX_mul (σ : Equiv.Perm (Fin N)) (j : Fin N)
    (f : MvPowerSeries (RepresentationTheory.LinearAlgebra.AuxiliaryPowerSeriesMatrix.AuxiliaryIndex N) k) (d : RepresentationTheory.LinearAlgebra.AuxiliaryPowerSeriesMatrix.AuxiliaryIndex N →₀ ℕ) :
...
/-- Computes the coefficient of the displayed product by testing agreement of the permutation-paired exponent entries. -/
theorem coeff_prod_invOfUnit_one_sub_X_mul_X_eq_ite (σ : Equiv.Perm (Fin N))
    (d : RepresentationTheory.LinearAlgebra.AuxiliaryPowerSeriesMatrix.AuxiliaryIndex N →₀ ℕ) :
    MvPowerSeries.coeff (R := k) d
      (∏ j : Fin N,
        MvPowerSeries.invOfUnit
...
/-- If a natural-valued indexing function is injective, the coefficient of the auxiliary power series at its associated auxiliary exponent index is one. -/
theorem auxiliarySeries_coeff_auxiliaryExponentIndex_eq_one_of_injective [CharZero k]
    (α : Fin N → ℕ) (hα : Function.Injective α) :
    MvPowerSeries.coeff (R := k) (auxiliaryExponentIndex N α) (RepresentationTheory.LinearAlgebra.AuxiliaryPowerSeriesMatrix.auxiliaryDeterminantPowerSeries N k) = 1 := by
  simp only [RepresentationTheory.LinearAlgebra.AuxiliaryPowerSeriesMatrix.auxiliaryDeterminantPowerSeries, map_sum]
  have key : ∀ σ : Equiv.Perm (Fin N),
      MvPowerSeries.coeff (R := k) (auxiliaryExponentIndex N α)
    ...
    intro σ
    rw [MvPowerSeries.coeff_C_mul, coeff_prod_invOfUnit_one_sub_X_mul_X_eq_ite N k σ (auxiliaryExponentIndex N α)]
    simp only [auxiliaryExponentIndex_apply_inl, auxiliaryExponentIndex_apply_inr]
  simp_rw [key]
...
end RepresentationTheory.MvPowerSeries.AuxiliaryExponentCoefficients
