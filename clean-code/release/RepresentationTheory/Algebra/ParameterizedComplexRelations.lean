...
/-! # Parameterized complex relations -/

namespace RepresentationTheory.Algebra.ParameterizedComplexRelations
...
inductive Relations (q : ℂˣ) : FreeAlgebra ℂ Generator → FreeAlgebra ℂ Generator → Prop
  | KL : Relations q (auxiliaryFreeAlgebraElementOne * auxiliaryFreeAlgebraElementTwo) 1
  | LK : Relations q (auxiliaryFreeAlgebraElementTwo * auxiliaryFreeAlgebraElementOne) 1
  | Ke : Relations q (auxiliaryFreeAlgebraElementOne * auxiliaryFreeAlgebraElementThree) (((q : ℂ) ^ 2) • (auxiliaryFreeAlgebraElementThree * auxiliaryFreeAlgebraElementOne))
  | Kf : Relations q (auxiliaryFreeAlgebraElementOne * auxiliaryFreeAlgebraElementFour) (((q : ℂ) ^ 2)⁻¹ • (auxiliaryFreeAlgebraElementFour * auxiliaryFreeAlgebraElementOne))
...
@[simp, source_ref "Chapter2/Problem2.16.5" (role := supporting)] lemma weightElement_mul_inverseWeightElement (q : ℂˣ) : weightElement q * inverseWeightElement q = 1 := by
  have h := RingQuot.mkAlgHom_rel ℂ (Relations.KL (q := q)); simp only [map_mul, map_one] at h; exact h


/-- The inverse weight element multiplied by the weight element is one. -/
@[simp, source_ref "Chapter2/Problem2.16.5" (role := supporting)] lemma inverseWeightElement_mul_weightElement (q : ℂˣ) : inverseWeightElement q * weightElement q = 1 := by
  have h := RingQuot.mkAlgHom_rel ℂ (Relations.LK (q := q)); simp only [map_mul, map_one] at h; exact h


/-- Moving the weight element past the raising element introduces the square of the parameter. -/
@[simp, source_ref "Chapter2/Problem2.16.5" (role := supporting)] lemma weightElement_mul_raisingElement (q : ℂˣ) : weightElement q * raisingElement q = ((q : ℂ) ^ 2) • (raisingElement q * weightElement q) := by
  have h := RingQuot.mkAlgHom_rel ℂ (Relations.Ke (q := q)); simp only [map_mul, map_smul] at h; exact h


/-- Moving the weight element past the lowering element introduces the inverse square of the parameter. -/
@[simp, source_ref "Chapter2/Problem2.16.5" (role := supporting)] lemma weightElement_mul_loweringElement (q : ℂˣ) : weightElement q * loweringElement q = ((q : ℂ) ^ 2)⁻¹ • (loweringElement q * weightElement q) := by
  have h := RingQuot.mkAlgHom_rel ℂ (Relations.Kf (q := q)); simp only [map_mul, map_smul] at h; exact h
...
@[simp, source_ref "Chapter2/Problem2.16.5" (role := supporting)] lemma parameterDifference_smul_commutator (q : ℂˣ) :
    ((q : ℂ) - (q : ℂ)⁻¹) • (raisingElement q * loweringElement q - loweringElement q * raisingElement q) = weightElement q - inverseWeightElement q := by
  have h := RingQuot.mkAlgHom_rel ℂ (Relations.ef (q := q))
...
private noncomputable def augmentationGen : Generator → ℂ := ![0, 0, 1, 1]


private noncomputable def augmentationFree : FreeAlgebra ℂ Generator →ₐ[ℂ] ℂ :=
  FreeAlgebra.lift ℂ augmentationGen

private theorem augmentationFree_rel (q : ℂˣ) :
    ∀ ⦃a b⦄, Relations q a b → augmentationFree a = augmentationFree b := by
  rintro _ _ h
...
end RepresentationTheory.Algebra.ParameterizedComplexRelations


attribute [nolint defsWithUnderscore]
  RepresentationTheory.Algebra.ParameterizedComplexRelations.Generator
  RepresentationTheory.Algebra.ParameterizedComplexRelations.auxiliaryFreeAlgebraElementThree
  ...
  RepresentationTheory.Algebra.ParameterizedComplexRelations.weightElement
  RepresentationTheory.Algebra.ParameterizedComplexRelations.inverseWeightElement
  RepresentationTheory.Algebra.ParameterizedComplexRelations.complexCharacter
  RepresentationTheory.Algebra.ParameterizedComplexRelations.weightActionEnd
  RepresentationTheory.Algebra.ParameterizedComplexRelations.loweringIterate
  RepresentationTheory.Algebra.ParameterizedComplexRelations.loweringWeight
  RepresentationTheory.Algebra.ParameterizedComplexRelations.raisingOnLoweringPowerCoeff
  RepresentationTheory.Algebra.ParameterizedComplexRelations.algebraActionEnd
  RepresentationTheory.Algebra.ParameterizedComplexRelations.raisingIterate
  RepresentationTheory.Algebra.ParameterizedComplexRelations.raisingWeight
  RepresentationTheory.Algebra.ParameterizedComplexRelations.loweringOnRaisingPowerCoeff
