...
namespace RepresentationTheory.Algebra.FiniteOrderShiftWeightModules
...
theorem inverseCyclicShiftEnd_comp_cyclicShiftEnd : (inverseCyclicShiftEnd α N).comp (cyclicShiftEnd α N) = LinearMap.id := by
  ext f k
  simp only [LinearMap.comp_apply, inverseCyclicShiftEnd, cyclicShiftEnd, LinearMap.coe_mk, AddHom.coe_mk, LinearMap.id_coe,
    id]
  rw [add_sub_cancel_right k 1, smul_smul, inv_mul_cancel₀ (unitIndexedScalar_ne_zero α N (k + 1)), one_smul]
/-- The cyclic shift composed with its inverse is the identity. -/


theorem cyclicShiftEnd_comp_inverseCyclicShiftEnd : (cyclicShiftEnd α N).comp (inverseCyclicShiftEnd α N) = LinearMap.id := by
  ext f k
  simp only [LinearMap.comp_apply, inverseCyclicShiftEnd, cyclicShiftEnd, LinearMap.coe_mk, AddHom.coe_mk, LinearMap.id_coe,
    id]
...
@[simp] theorem cyclicShiftUnit_val : (cyclicShiftUnit α N : Module.End ℂ (Fin N → ℂ)) = cyclicShiftEnd α N := rfl
...
theorem inverseDiagonalWeightEnd_comp_diagonalWeightEnd : (inverseDiagonalWeightEnd q β N).comp (diagonalWeightEnd q β N) = LinearMap.id := by
  ext f k
  simp only [LinearMap.comp_apply, inverseDiagonalWeightEnd, diagonalWeightEnd, LinearMap.coe_mk, AddHom.coe_mk, LinearMap.id_coe,
    id]
...
theorem diagonalWeightEnd_comp_inverseDiagonalWeightEnd : (diagonalWeightEnd q β N).comp (inverseDiagonalWeightEnd q β N) = LinearMap.id := by
  ext f k
  simp only [LinearMap.comp_apply, inverseDiagonalWeightEnd, diagonalWeightEnd, LinearMap.coe_mk, AddHom.coe_mk, LinearMap.id_coe,
    id]
...
@[simp] theorem diagonalWeightUnit_val : (diagonalWeightUnit q β N : Module.End ℂ (Fin N → ℂ)) = diagonalWeightEnd q β N := rfl
...
theorem cyclicShiftEnd_single (m : Fin N) :
    cyclicShiftEnd α N (Pi.single m (1 : ℂ)) = unitIndexedScalar α N (m + 1) • Pi.single (m + 1) (1 : ℂ) := by
  classical
  funext i
...
theorem diagonalWeightEnd_single (m : Fin N) :
    diagonalWeightEnd q β N (Pi.single m (1 : ℂ)) = twoUnitIndexedScalar q β N m • Pi.single m (1 : ℂ) := by
  classical
  funext i
...
theorem diagonalWeightEnd_mul_cyclicShiftEnd (hqorder : orderOf q = N) :
    (diagonalWeightEnd q β N) * (cyclicShiftEnd α N) = (q : ℂ) • ((cyclicShiftEnd α N) * (diagonalWeightEnd q β N)) := by
  refine LinearMap.ext fun f => ?_
  funext k
  simp only [Module.End.mul_apply, LinearMap.smul_apply, cyclicShiftEnd, diagonalWeightEnd, twoUnitIndexedScalar, LinearMap.coe_mk,
...
theorem diagonalWeightUnit_val_mul_cyclicShiftUnit_val (hqorder : orderOf q = N) :
    ((diagonalWeightUnit q β N : Module.End ℂ (Fin N → ℂ))) * (cyclicShiftUnit α N : Module.End ℂ (Fin N → ℂ))
      = (q : ℂ) • ((cyclicShiftUnit α N : Module.End ℂ (Fin N → ℂ))
          * (diagonalWeightUnit q β N : Module.End ℂ (Fin N → ℂ))) := by
  rw [cyclicShiftUnit_val, diagonalWeightUnit_val]; exact diagonalWeightEnd_mul_cyclicShiftEnd q α β N hqorder
...
theorem firstGenerator_smul (hqorder : orderOf q = N) (f : Fin N → ℂ) :
    ...
    (RepresentationTheory.QuantumTorus.Representations.monomial q (1, 0)) • f = cyclicShiftEnd α N f := by
  letI := finiteOrderModule q α β N hqorder
  rw [RepresentationTheory.QuantumTorus.Representations.moduleOfQCommute_smul_eq_representation_apply]
  change RepresentationTheory.QuantumTorus.Representations.representationOfQCommute q (cyclicShiftUnit α N) (diagonalWeightUnit q β N) _ (RepresentationTheory.QuantumTorus.Representations.monomial q (1, 0)) f = _
  rw [RepresentationTheory.QuantumTorus.Representations.representationOfQCommute_firstGenerator, cyclicShiftUnit_val]
...
theorem secondGenerator_smul (hqorder : orderOf q = N) (f : Fin N → ℂ) :
    letI := finiteOrderModule q α β N hqorder
    (RepresentationTheory.QuantumTorus.Representations.monomial q (0, 1)) • f = diagonalWeightEnd q β N f := by
  letI := finiteOrderModule q α β N hqorder
  rw [RepresentationTheory.QuantumTorus.Representations.moduleOfQCommute_smul_eq_representation_apply]
  change RepresentationTheory.QuantumTorus.Representations.representationOfQCommute q (cyclicShiftUnit α N) (diagonalWeightUnit q β N) _ (RepresentationTheory.QuantumTorus.Representations.monomial q (0, 1)) f = _
  rw [RepresentationTheory.QuantumTorus.Representations.representationOfQCommute_secondGenerator, diagonalWeightUnit_val]
...
theorem prod_unitIndexedScalar (k : Fin N) :
  ...
  rw [← Fin.prod_univ_eq_prod_range (fun i : ℕ => unitIndexedScalar α N (k - i • (1 : Fin N))) N]
  have h : ∀ i : Fin N, unitIndexedScalar α N (k - (i : ℕ) • (1 : Fin N)) = unitIndexedScalar α N (Equiv.subLeft k i) := by
...
theorem cyclicShiftEnd_pow_apply (m : ℕ) (f : Fin N → ℂ) (k : Fin N) :
    ((cyclicShiftEnd α N) ^ m) f k
      = (∏ i ∈ Finset.range m, unitIndexedScalar α N (k - i • (1 : Fin N))) • f (k - m • (1 : Fin N)) := by
...
theorem cyclicShiftEnd_pow_card : (cyclicShiftEnd α N) ^ N = (α : ℂ) • (1 : Module.End ℂ (Fin N → ℂ)) := by
  refine LinearMap.ext fun f => ?_
  funext k
...
theorem diagonalWeightEnd_pow_card (hqorder : orderOf q = N) :
    (diagonalWeightEnd q β N) ^ N = ((β : ℂ) ^ N) • (1 : Module.End ℂ (Fin N → ℂ)) := by
  refine LinearMap.ext fun f => ?_
  funext k
...
theorem firstGenerator_cardExponent_smul (hqorder : orderOf q = N) (f : Fin N → ℂ) :
    letI := finiteOrderModule q α β N hqorder
    (RepresentationTheory.QuantumTorus.Representations.monomial q ((N : ℤ), 0)) • f = (α : ℂ) • f := by
  letI := finiteOrderModule q α β N hqorder
  rw [RepresentationTheory.QuantumTorus.Representations.moduleOfQCommute_smul_eq_representation_apply]
  change RepresentationTheory.QuantumTorus.Representations.representationOfQCommute q (cyclicShiftUnit α N) (diagonalWeightUnit q β N) _
    (RepresentationTheory.QuantumTorus.Representations.monomial q ((N : ℤ), 0)) f = _
  rw [RepresentationTheory.QuantumTorus.Representations.representationOfQCommute_monomial]
  simp only [RepresentationTheory.QuantumTorus.Representations.monomialEnd, zpow_zero, Units.val_one, mul_one, zpow_natCast,
...
theorem secondGenerator_cardExponent_smul (hqorder : orderOf q = N) (f : Fin N → ℂ) :
    letI := finiteOrderModule q α β N hqorder
    (RepresentationTheory.QuantumTorus.Representations.monomial q (0, (N : ℤ))) • f = ((β : ℂ) ^ N) • f := by
  letI := finiteOrderModule q α β N hqorder
  rw [RepresentationTheory.QuantumTorus.Representations.moduleOfQCommute_smul_eq_representation_apply]
  change RepresentationTheory.QuantumTorus.Representations.representationOfQCommute q (cyclicShiftUnit α N) (diagonalWeightUnit q β N) _
    (RepresentationTheory.QuantumTorus.Representations.monomial q (0, (N : ℤ))) f = _
  rw [RepresentationTheory.QuantumTorus.Representations.representationOfQCommute_monomial]
  simp only [RepresentationTheory.QuantumTorus.Representations.monomialEnd, zpow_zero, Units.val_one, one_mul, zpow_natCast,
    Units.val_pow_eq_pow_val, diagonalWeightUnit_val, diagonalWeightEnd_pow_card q β N hqorder]
  simp
...
theorem firstGenerator_pow_card_smul (hqorder : orderOf q = N) (f : Fin N → ℂ) :
    letI := finiteOrderModule q α β N hqorder
    ((RepresentationTheory.QuantumTorus.Representations.monomial q (1, 0)) ^ N) • f = (α : ℂ) • f := by
  rw [RepresentationTheory.QuantumTorus.Representations.monomial_firstExponent_pow, one_mul]
...
theorem secondGenerator_pow_card_smul (hqorder : orderOf q = N) (f : Fin N → ℂ) :
    letI := finiteOrderModule q α β N hqorder
    ((RepresentationTheory.QuantumTorus.Representations.monomial q (0, 1)) ^ N) • f = ((β : ℂ) ^ N) • f := by
  rw [RepresentationTheory.QuantumTorus.Representations.monomial_secondExponent_pow, one_mul]
  exact secondGenerator_cardExponent_smul q α β N hqorder f

end Family

end RepresentationTheory.Algebra.FiniteOrderShiftWeightModules



attribute [nolint defsWithUnderscore]
  RepresentationTheory.Algebra.FiniteOrderShiftWeightModules.unitIndexedScalar RepresentationTheory.Algebra.FiniteOrderShiftWeightModules.cyclicShiftEnd
  RepresentationTheory.Algebra.FiniteOrderShiftWeightModules.inverseCyclicShiftEnd RepresentationTheory.Algebra.FiniteOrderShiftWeightModules.cyclicShiftUnit
  RepresentationTheory.Algebra.FiniteOrderShiftWeightModules.twoUnitIndexedScalar RepresentationTheory.Algebra.FiniteOrderShiftWeightModules.diagonalWeightEnd
  RepresentationTheory.Algebra.FiniteOrderShiftWeightModules.inverseDiagonalWeightEnd RepresentationTheory.Algebra.FiniteOrderShiftWeightModules.diagonalWeightUnit
  RepresentationTheory.Algebra.FiniteOrderShiftWeightModules.finiteOrderModule
