...
namespace RepresentationTheory.Algebra.Lie.AuxiliaryEndomorphismKernelProfiles
...
/-- An auxiliary complex-linear endomorphism of the displayed Lie module. -/
noncomputable abbrev auxiliaryEndomorphism (V : Type*) [AddCommGroup V] [Module ℂ V]
...
/-- A Lie-module equivalence intertwines the auxiliary endomorphisms of its source and target. -/
theorem auxiliaryEndomorphism_intertwines (φ : V ≃ₗ⁅ℂ,_root_.RepresentationTheory.Algebra.Lie.ComplexTwoByTwoMatrices.complexTwoByTwoMatrixLieSubalgebra⁆ W) :
    auxiliaryEndomorphism W ∘ₗ (φ.toLinearEquiv : V →ₗ[ℂ] W)
      = (φ.toLinearEquiv : V →ₗ[ℂ] W) ∘ₗ auxiliaryEndomorphism V := by
  ext v
  simp only [LinearMap.comp_apply, LinearEquiv.coe_coe, LieModuleEquiv.coe_toLinearEquiv,
    auxiliaryEndomorphism, LieModule.toEnd_apply_apply]
...
/-- Equivalent Lie modules have equal dimensions of the kernels of every power of their auxiliary endomorphisms. -/
theorem finrank_ker_auxiliaryEndomorphism_pow_eq_of_equiv (φ : V ≃ₗ⁅ℂ,_root_.RepresentationTheory.Algebra.Lie.ComplexTwoByTwoMatrices.complexTwoByTwoMatrixLieSubalgebra⁆ W) (k : ℕ) :
    Module.finrank ℂ (LinearMap.ker (auxiliaryEndomorphism V ^ k))
      = Module.finrank ℂ (LinearMap.ker (auxiliaryEndomorphism W ^ k)) :=
  finrank_ker_eq_of_intertwining_equiv φ.toLinearEquiv (auxiliaryEndomorphism V ^ k) (auxiliaryEndomorphism W ^ k)
    (pow_intertwines_of_intertwines φ.toLinearEquiv (auxiliaryEndomorphism V) (auxiliaryEndomorphism W) (auxiliaryEndomorphism_intertwines φ) k)

end NullSeq

/-- On the finite complex function space, the auxiliary endomorphism is evaluation of the displayed representation at the displayed element. -/
theorem auxiliaryEndomorphism_finFunction (d : ℕ) : auxiliaryEndomorphism (Fin d → ℂ) = _root_.RepresentationTheory.LieAlgebra.Sl2Representations.finFunctionRepresentation d _root_.RepresentationTheory.LieAlgebra.Sl2Representations.raisingElement := by
  refine LinearMap.ext fun v => ?_
  rw [auxiliaryEndomorphism, LieModule.toEnd_apply_apply]
  rfl

/-- The kernel of the displayed endomorphism power on complex-valued functions on `Fin d` has dimension `min k d`. -/
theorem finrank_ker_auxiliaryEndomorphism_finFunction_pow (d k : ℕ) :
    Module.finrank ℂ (LinearMap.ker (auxiliaryEndomorphism (Fin d → ℂ) ^ k)) = min k d := by
  ...
  rw [auxiliaryEndomorphism_finFunction,
...
/-- The auxiliary endomorphism on a dependent function module is the pointwise family of component endomorphisms. -/
theorem auxiliaryEndomorphism_pi : auxiliaryEndomorphism (∀ i, W i) = LinearMap.piMap fun i => auxiliaryEndomorphism (W i) := by
  ext v j
  simp only [auxiliaryEndomorphism, LieModule.toEnd_apply_apply, LinearMap.coe_piMap, Pi.map_apply,
    _root_.RepresentationTheory.LieAlgebra.ModuleProducts.bracket_pi_apply]

/-- For a finite dependent product, the kernel dimension of a power of the auxiliary endomorphism is the sum of the component kernel dimensions. -/
theorem finrank_ker_auxiliaryEndomorphism_pi_pow [∀ i, FiniteDimensional ℂ (W i)] (k : ℕ) :
    Module.finrank ℂ (LinearMap.ker (auxiliaryEndomorphism (∀ i, W i) ^ k))
      = ∑ i, Module.finrank ℂ (LinearMap.ker (auxiliaryEndomorphism (W i) ^ k)) := by
  rw [auxiliaryEndomorphism_pi, _root_.RepresentationTheory.LinearAlgebra.KernelDimensionProfiles.finrank_piMap_pow_kernel]

end Pi

/-- For the displayed dependent complex function space, the kernel dimension is the sum of `min k (n i + 1)` over the index type. -/
theorem finrank_ker_auxiliaryEndomorphism_finSuccPi_pow {ι : Type*} [Fintype ι] (n : ι → ℕ) (k : ℕ) :
    Module.finrank ℂ (LinearMap.ker (auxiliaryEndomorphism (∀ i, (Fin (n i + 1) → ℂ)) ^ k))
      = ∑ i, min k (n i + 1) := by
  rw [finrank_ker_auxiliaryEndomorphism_pi_pow]
  exact Finset.sum_congr rfl fun i _ => finrank_ker_auxiliaryEndomorphism_finFunction_pow (n i + 1) k

section Reindex

variable {ι κ : Type*}

/-- Equality of two natural-number parameters induces a Lie-module equivalence between the corresponding finite complex function spaces. -/
noncomputable def finSuccFunctionLieModuleEquivOfEq {a b : ℕ} (h : a = b) :
...
/-- An auxiliary multiset of natural numbers associated with a natural-number-valued function on a finite type. -/
def auxiliaryNatMultiset {ι : Type*} [Fintype ι] (n : ι → ℕ) : Multiset ℕ :=
...
theorem zero_not_mem_auxiliaryNatMultiset {ι : Type*} [Fintype ι] (n : ι → ℕ) :
    (0 : ℕ) ∉ auxiliaryNatMultiset n := by
  simp only [auxiliaryNatMultiset, Multiset.mem_map]
  rintro ⟨i, -, hi⟩
  omega

/-- Two finite-dimensional modules for the displayed Lie algebra are equivalent when all powers of their auxiliary endomorphisms have kernels of equal dimension. -/
theorem nonempty_equiv_of_auxiliaryEndomorphism_kernelProfile_eq
    ...
    (h : ∀ k, Module.finrank ℂ (LinearMap.ker (auxiliaryEndomorphism V ^ k))
      = Module.finrank ℂ (LinearMap.ker (auxiliaryEndomorphism W ^ k))) :
  ...
  refine ⟨eV.trans (((piLieModuleEquiv fun i => finSuccFunctionLieModuleEquivOfEq (hσ' i)).trans
    (reindexPiLieModuleEquiv σ (fun j => (Fin (nW j + 1) → ℂ))).symm).trans eW.symm)⟩

end RepresentationTheory.Algebra.Lie.AuxiliaryEndomorphismKernelProfiles
