...
namespace RepresentationTheory.LieAlgebra.FiniteDimensionalModels
...
theorem sequence_has_property_of_characteristicAssumptions
...
private theorem _root_.RepresentationTheory.LieAlgebra.BigradedCocycleLifts.IsAlternatingLieCocycle.eq_imaginaryCoboundary_loopFam
  ...
  · rw [hw.apply_eq_zero_of_not_compatible hIJ]
    exact (_root_.RepresentationTheory.LieAlgebra.BigradedCocycleLifts.SpecialBinaryFormCondition.apply_eq_zero_of_not_compatible
      (sequence_has_property_of_characteristicAssumptions h2 h3 h5 _) hIJ).symm
...
theorem hasStructure_of_conditions
    (c : _root_.RepresentationTheory.LieAlgebra.ExplicitConstructions.matrixPolynomialLieSubalgebra k → _root_.RepresentationTheory.LieAlgebra.ExplicitConstructions.matrixPolynomialLieSubalgebra k → k) (hc : _root_.RepresentationTheory.LieAlgebra.BigradedCocycleLifts.IsAlternatingLieCocycle k c)
...
@[simp] theorem specifiedScalar_eq_zero (m : ℕ) : _root_.RepresentationTheory.LieAlgebra.AuxiliaryBracketCalculus.auxiliaryCentralFamily k m = 0 :=
...
theorem derivedPair_has_property (m : ℕ) :
  ...
  _root_.RepresentationTheory.LieAlgebra.AuxiliaryBracketCalculus.auxiliaryPairCondition_companion_sequence (by norm_num) (by norm_num) (by norm_num) specifiedScalar_eq_zero m

/-- The displayed linear map from the finite-dimensional model is injective. -/
theorem modelMap_injective : Function.Injective (_root_.RepresentationTheory.LieAlgebra.ExplicitConstructions.linearMap_aux2 k) :=
  (_root_.RepresentationTheory.LinearMap.KernelDecomposition.injective_iff_auxFamily_eq_zero (by norm_num) (by norm_num) (by norm_num)).2
    specifiedScalar_eq_zero

/-- The range of the displayed map spans the whole ambient module. -/
theorem span_range_modelMap_eq_top :
...
/-- Computes the four-dimensional basis vector at an index. -/
@[simp] theorem fourDimensionalBasis_apply (I : _root_.RepresentationTheory.LieAlgebra.ExplicitConstructions.AuxiliaryIndex) : fourDimensionalBasis (k := k) I = _root_.RepresentationTheory.LieAlgebra.AuxiliaryBracketCalculus.indexedFamily k I :=
  Module.Basis.mk_apply _ _ _

/-- The displayed four-dimensional-indexed model is not finite as a module over the field. -/
theorem not_moduleFinite_fourDimensionalModel : ¬ Module.Finite k (_root_.RepresentationTheory.LieAlgebra.ExplicitConstructions.AuxiliaryType k 4) := by
  letI : Infinite _root_.RepresentationTheory.LieAlgebra.ExplicitConstructions.AuxiliaryIndex := Infinite.of_injective (fun m : ℕ ↦ _root_.RepresentationTheory.LieAlgebra.ExplicitConstructions.AuxiliaryIndex.odd m 0) <| by
    ...
    cases h
    rfl
  exact Module.not_finite_of_infinite_basis (fourDimensionalBasis (k := k))

/-- A Lie algebra equivalence from the displayed four-dimensional model to the specified subtype. -/
noncomputable def fourDimensionalLieEquiv : _root_.RepresentationTheory.LieAlgebra.ExplicitConstructions.AuxiliaryType k 4 ≃ₗ⁅k⁆ _root_.RepresentationTheory.LieAlgebra.ExplicitConstructions.matrixPolynomialLieSubalgebra k where
...
/-- Computes the Lie equivalence on an element of the four-dimensional model. -/
@[simp] theorem fourDimensionalLieEquiv_apply (u : _root_.RepresentationTheory.LieAlgebra.ExplicitConstructions.AuxiliaryType k 4) :
    fourDimensionalLieEquiv (k := k) u = _root_.RepresentationTheory.LieAlgebra.BigradedCocycleLifts.projectToDistinguishedSubspace k u := by
...
end RepresentationTheory.LieAlgebra.FiniteDimensionalModels
attribute [nolint defsWithUnderscore]
  RepresentationTheory.LieAlgebra.FiniteDimensionalModels.fourDimensionalBasis
  RepresentationTheory.LieAlgebra.FiniteDimensionalModels.fourDimensionalLieEquiv
