...
def zeroToSink : @Quiver.Hom (Fin 4) finFourQuiverA 0 3 := ⟨⟨by decide, by decide⟩⟩
/-- The distinguished quiver morphism from vertex one to the sink. -/
def oneToSink : @Quiver.Hom (Fin 4) finFourQuiverA 1 3 := ⟨⟨by decide, by decide⟩⟩
/-- The distinguished quiver morphism from vertex two to the sink. -/
def twoToSink : @Quiver.Hom (Fin 4) finFourQuiverA 2 3 := ⟨⟨by decide, by decide⟩⟩

/-- The complex-linear map carried by the arrow from vertex zero to the sink. -/
noncomputable abbrev zeroToSinkLinearMap :=
  @RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.map ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt
    0 3 zeroToSink
...
theorem hom_eq_zeroToSink_or_oneToSink_or_twoToSink {a b : Fin 4} (e : @Quiver.Hom (Fin 4) finFourQuiverA a b) :
...
private theorem centre_finrank : Module.finrank ℂ
    (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt 3) = 2 := by
  have h := finFourDiagramAAlt_finrank_eq 3
  have h' : (Module.finrank ℂ
      (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt 3) : ℤ) = 2 := by
    simpa using h
  exact_mod_cast h'

private theorem arm_finrank (i : Fin 4) (hi : i ≠ 3) : Module.finrank ℂ
    (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt i) = 1 := by
  have h := finFourDiagramAAlt_finrank_eq i
  fin_cases i <;> simp_all

private theorem armMap_injective {i : Fin 4} (hi : i ≠ 3)
    (e : @Quiver.Hom (Fin 4) finFourQuiverA i 3) : Function.Injective
      (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.map ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt
        i 3 e) := by
  let A := @RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.map ℂ (Fin 4) _ finFourQuiverA
    finFourDiagramAAlt i 3 e
  ...
  have hP : ∀ {a b : Fin 4} (f : @Quiver.Hom (Fin 4) finFourQuiverA a b), ∀ x ∈ P a,
      @RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.map ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt
        a b f x ∈ P b := by
    intro a b f x hx
    by_cases ha : a = i
    ...
    · have hxzero : x = 0 := by
        have : x ∈ (⊥ : Submodule ℂ
            (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt a)) := by
          simpa [P, ha] using hx
        exact (Submodule.mem_bot (R := ℂ)).mp this
      subst x
      simp [P]
  have hR : ∀ {a b : Fin 4} (f : @Quiver.Hom (Fin 4) finFourQuiverA a b), ∀ x ∈ R a,
      @RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.map ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt
        a b f x ∈ R b := by
    intro a b f x hx
    rcases hom_eq_zeroToSink_or_oneToSink_or_twoToSink f with ⟨_, rfl, _⟩ | ⟨_, rfl, _⟩ | ⟨_, rfl, _⟩ <;>
  ...
  rcases finFourDiagramAAlt_hasProperty.2 P R hP hR hcompl with hbot | hbot
  · exact LinearMap.ker_eq_bot.mp (by simpa [P] using hbot i)
  · exfalso
    have htop : (⊤ : Submodule ℂ
        (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt 3)) = ⊥ := by
      simpa [R, h3i] using hbot 3
    have hrank := congrArg (fun T : Submodule ℂ
        (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt 3) =>
...
theorem sup_range_eq_top :
  ...
  let A₁ := @RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.map ℂ (Fin 4) _ finFourQuiverA
    finFourDiagramAAlt 0 3 zeroToSink
  let A₂ := @RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.map ℂ (Fin 4) _ finFourQuiverA
    finFourDiagramAAlt 1 3 oneToSink
  let A₃ := @RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.map ℂ (Fin 4) _ finFourQuiverA
    finFourDiagramAAlt 2 3 twoToSink
  ...
  rcases finFourDiagramAAlt_hasProperty.2 P R hP hR hcompl with hbot | hbot
  · have htop : (⊤ : Submodule ℂ
        (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt 0)) = ⊥ := by
      simpa [P] using hbot 0
    have hrank := congrArg (fun U : Submodule ℂ
        (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt 0) =>
...
structure AuxiliaryRangeEquivData where
  ...
  sinkEquiv : (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA
    finFourDiagramAAlt 3) ≃ₗ[ℂ]
    (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA finFourDiagramAAlt 3)
  /-- Identifies the space at vertex zero with the image of its map into the sink. -/
  zeroEquivRange : (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA
    finFourDiagramAAlt 0) ≃ₗ[ℂ] LinearMap.range zeroToSinkLinearMap
  /-- Identifies the space at vertex one with the image of its map into the sink. -/
  oneEquivRange : (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA
    finFourDiagramAAlt 1) ≃ₗ[ℂ] LinearMap.range oneToSinkLinearMap
  /-- Identifies the space at vertex two with the image of its map into the sink. -/
  twoEquivRange : (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA
    finFourDiagramAAlt 2) ≃ₗ[ℂ] LinearMap.range twoToSinkLinearMap
...
theorem finrank_ranges_and_sup_eq_top :
    Module.finrank ℂ (@RepresentationTheory.CategoryTheory.QuiverLinearDiagrams.QuiverLinearDiagram.obj ℂ (Fin 4) _ finFourQuiverA
      finFourDiagramAAlt 3) = 2 ∧
