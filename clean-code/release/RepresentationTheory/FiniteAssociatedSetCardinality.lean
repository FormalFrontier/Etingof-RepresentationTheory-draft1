...
/-!
# Finite associated-set cardinality

Finiteness and cardinality for a set determined by a finite integer matrix model.
-/

namespace RepresentationTheory.FiniteAssociatedSetCardinality
...
private lemma An_cartan_entry (n : ℕ) (hn : 1 ≤ n) (k j : Fin n) :
    (2 • (1 : Matrix (Fin n) (Fin n) ℤ) - (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix) k j =
    if k = j then 2
...
private lemma An_qform_peel (m : ℕ) (hm : 1 ≤ m) (x : Fin (m + 1) → ℤ) :
    dotProduct x ((2 • (1 : Matrix (Fin (m + 1)) (Fin (m + 1)) ℤ) -
      (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A (m + 1) (by omega)).matrix).mulVec x) =
    dotProduct (fun i : Fin m => x ⟨i.val, by omega⟩)
      ((2 • (1 : Matrix (Fin m) (Fin m) ℤ) -
        (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm).matrix).mulVec (fun i : Fin m => x ⟨i.val, by omega⟩)) +
...
private lemma An_qform_ge_endpoints : ∀ (n : ℕ) (hn : 1 ≤ n) (x : Fin n → ℤ),
    dotProduct x ((2 • (1 : Matrix (Fin n) (Fin n) ℤ) -
      (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix).mulVec x) ≥
  ...
  | succ m ih =>
    ...
    ·
      ...
      simp only [dotProduct, mulVec, RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.matrix, Matrix.sub_apply,
...
private lemma An_qform_zero : ∀ (n : ℕ) (hn : 1 ≤ n) (x : Fin n → ℤ),
    (∀ i, 0 ≤ x i) →
    dotProduct x ((2 • (1 : Matrix (Fin n) (Fin n) ℤ) -
      (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix).mulVec x) = 0 →
    x = 0 := by
  intro n
  induction n with
  | zero => intro hn; omega
  | succ m ih =>
    ...
    ·
      ...
      have hqm_nonneg : dotProduct x' ((2 • (1 : Matrix (Fin m) (Fin m) ℤ) -
          (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm1).matrix).mulVec x') ≥ 0 := by
      ...
      ·
        have hqm_zero : dotProduct x' ((2 • (1 : Matrix (Fin m) (Fin m) ℤ) -
            (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm1).matrix).mulVec x') = 0 := by nlinarith [hxm_zero]
      ...
      ·
        ...
        have hqm_zero : dotProduct x' ((2 • (1 : Matrix (Fin m) (Fin m) ℤ) -
            (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm1).matrix).mulVec x') = 0 := by nlinarith
        have hx'_zero := ih hm1 x' hpos' hqm_zero
...
private lemma An_qform_no_double : ∀ (m : ℕ) (hm : 1 ≤ m) (x : Fin m → ℤ),
    ...
    dotProduct x ((2 • (1 : Matrix (Fin m) (Fin m) ℤ) -
      (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm).matrix).mulVec x) ≠ 4 := by
  intro m
  induction m with
  | zero => intro hm; omega
  | succ k ih =>
    ...
    ·
      ...
      set r := dotProduct x' ((2 • (1 : Matrix (Fin k) (Fin k) ℤ) -
          (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A k hk1).matrix).mulVec x')
...
private lemma An_bound (n : ℕ) (hn : 1 ≤ n) (x : Fin n → ℤ)
    (hr : RepresentationTheory.AuxiliaryIntegerMatrixVectorProperty.IsAuxiliaryForMatrix n (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix x)
  ...
  | succ m ih =>
    ...
    · have hm1 : 1 ≤ m := by omega
      ...
      set r := dotProduct x' ((2 • (1 : Matrix (Fin m) (Fin m) ℤ) -
          (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm1).matrix).mulVec x') with hr_def
      ...
      ·
        ...
        have hroot' : RepresentationTheory.AuxiliaryIntegerMatrixVectorProperty.IsAuxiliaryForMatrix m (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm1).matrix x' :=
          ⟨hne', hr2 ▸ hr_def ▸ rfl⟩
        intro ⟨j, hj⟩
        by_cases hjm : j < m
        ...
        · have hjm' : j < m := by omega
          ...
          ·
            ...
            have hne' : x' ≠ 0 := by
              ...
              linarith
            have hroot' : RepresentationTheory.AuxiliaryIntegerMatrixVectorProperty.IsAuxiliaryForMatrix m
                (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm1).matrix x' :=
              ⟨hne', hr2 ▸ hr_def ▸ rfl⟩
            exact ih hm1 x' hroot' hpos' hroot'.2 hroot'.1 ⟨j, hjm'⟩
...
private lemma An_ivec_qform_eq : ∀ (n : ℕ) (hn : 1 ≤ n)
    ...
    dotProduct (fun i : Fin n => ((ivec n a b i : Fin 2) : ℤ))
      ((2 • (1 : Matrix (Fin n) (Fin n) ℤ) -
        (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix).mulVec
  ...
  | succ m ih =>
    ...
    · subst hm0
      ...
      subst ha0; subst hb0
      simp only [dotProduct, mulVec, RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.matrix,
    ...
    · have hm1 : 1 ≤ m := by omega
      ...
      change dotProduct xi ((2 • (1 : Matrix (Fin (m + 1)) (Fin (m + 1)) ℤ) -
        (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A (m + 1) hn).matrix).mulVec xi) = 2
...
private lemma ivec_mem : ∀ (n : ℕ) (hn : 1 ≤ n) (a b : ℕ) (hab : a ≤ b) (hb : b < n),
    ivec n a b ∈ RepresentationTheory.MatrixBoundedVectors.boundedVectors n (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix 2 := by
  intro n; induction n with
  | zero => intro hn; omega
  | succ m ih =>
    intro hn a b hab hb
...
private lemma root_is_ivec : ∀ (n : ℕ) (hn : 1 ≤ n) (v : Fin n → Fin 2),
    v ∈ RepresentationTheory.MatrixBoundedVectors.boundedVectors n (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix 2 →
    ∃ a b : ℕ, a ≤ b ∧ b < n ∧ v = ivec n a b := by
  intro n; induction n with
  | zero => intro hn; omega
  | succ m ih =>
    intro hn v hv
    simp only [RepresentationTheory.MatrixBoundedVectors.boundedVectors, Finset.mem_filter, Finset.mem_univ,
    ...
    · have hm1 : 1 ≤ m := by omega
      ...
      have hq_peel : dotProduct (fun i : Fin m => (v' i : ℤ))
          ((2 • (1 : Matrix (Fin m) (Fin m) ℤ) -
            (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm1).matrix).mulVec
            (fun i : Fin m => (v' i : ℤ))) +
          2 * (v ⟨m, by omega⟩ : ℤ) ^ 2 -
      ...
      have mk_mem : ∀ (hne' : v' ≠ 0),
          dotProduct (fun i : Fin m => (v' i : ℤ))
            ((2 • (1 : Matrix (Fin m) (Fin m) ℤ) -
              (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm1).matrix).mulVec
              (fun i : Fin m => (v' i : ℤ))) = 2 →
          v' ∈ RepresentationTheory.MatrixBoundedVectors.boundedVectors m (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A m hm1).matrix 2 := by
        intro hne' hq'
        simp only [RepresentationTheory.MatrixBoundedVectors.boundedVectors, Finset.mem_filter, Finset.mem_univ,
...
private lemma An_count (n : ℕ) (hn : 1 ≤ n) :
    (RepresentationTheory.MatrixBoundedVectors.boundedVectors n (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix 2).card =
      n * (n + 1) / 2 := by
  have heq : RepresentationTheory.MatrixBoundedVectors.boundedVectors n (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix 2 =
...
/-- For every positive natural number n, the associated set is finite and has `n * (n + 1) / 2` elements. -/
lemma associatedSet_finite_and_ncard (n : ℕ) (hn : 1 ≤ n) :
    (RepresentationTheory.MatrixBoundedVectors.integerVectors n (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix).Finite ∧
    Set.ncard (RepresentationTheory.MatrixBoundedVectors.integerVectors n (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix) =
      n * (n + 1) / 2 := by
  obtain ⟨hfin, hcard⟩ := RepresentationTheory.MatrixBoundedVectors.integerVectors_finite_ncard_eq_boundedVectors_card (An_bound n hn)
...
/-- The set attached to a positive natural number n is finite, with cardinality `n * (n + 1) / 2`. -/
theorem finite_associatedSet_and_ncard_eq (n : ℕ) (hn : 1 ≤ n) :
    (RepresentationTheory.MatrixBoundedVectors.integerVectors n (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix).Finite ∧
    Set.ncard (RepresentationTheory.MatrixBoundedVectors.integerVectors n (RepresentationTheory.FiniteIntegerMatrixModels.FiniteMatrixModel.A n hn).matrix) =
      n * (n + 1) / 2 :=
  associatedSet_finite_and_ncard n hn

end RepresentationTheory.FiniteAssociatedSetCardinality
