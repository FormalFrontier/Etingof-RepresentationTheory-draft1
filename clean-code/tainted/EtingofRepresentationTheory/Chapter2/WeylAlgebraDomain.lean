...
The PBW monomials `x^i y^j` are ordered lexicographically, with the `x`-degree first.  Reordering
a product of two PBW monomials changes its leading term only by terms of smaller `x`-degree:

`(x^i y^j) (x^k y^l) = x^(i+k) y^(j+l) + lower terms`.

Consequently the leading coefficient of a product is the product of the leading coefficients.
This proves that the Weyl algebra over a domain has no zero divisors.  The PBW basis used below is
the characteristic-free basis from `FaithfulWeylModule`.
-/
...
/-- The characteristic-free PBW basis, equipped with the lexicographic order on exponents. -/
...
/-- The PBW coefficient at a lexicographically indexed monomial. -/
...
/-- The span of PBW monomials whose lexicographic exponent is at most `p`. -/
...
/-- The leading PBW coefficient of a product is the product of the leading coefficients. -/
...
/-- The Weyl algebra over a commutative domain has no zero divisors.  No characteristic
