...
`isNilpotent_or_isUnit_of_finiteLength_indecomposable` and its sum-form corollary
`isNilpotent_sum_of_finiteLength_indecomposable`; the projection/complement bookkeeping is pure
module theory and carries over verbatim.
-/
...
/-- **Sum form of Fitting's lemma for finite-length modules.** A finite sum of nilpotent
endomorphisms of a finite-length indecomposable module is again nilpotent. This is the
finite-length analogue of Lemma 3.8.2(ii) (`Etingof.sum_nilpotent_endo_indecomposable`), and it is
the combinatorial input to the Krull-Schmidt-Azumaya exchange: it forces at least one summand of a
sum that equals `id` to be a unit.

The argument is the same local-ring bookkeeping as the finite-dimensional proof, but the
underlying dichotomy is `isNilpotent_or_isUnit_of_finiteLength_indecomposable` (no ground field is
needed). -/
theorem isNilpotent_sum_of_finiteLength_indecomposable
    [IsArtinian A V] [IsNoetherian A V]
    (hV : Etingof.IsIndecomposable A V)
  ...
  | succ n ih =>
    ...
    rcases isNilpotent_or_isUnit_of_finiteLength_indecomposable hV
    ...
    · exfalso
      ...
      have h_nilp0 : IsNilpotent ((↑u⁻¹ : Module.End A V) * θ 0) := by
        rcases isNilpotent_or_isUnit_of_finiteLength_indecomposable hV (↑u⁻¹ * θ 0) with hn | hb
        · exact hn
...
/-- Two complements of the same submodule are isomorphic: if `M ⊕ C = V = M ⊕ D` then
`C ≃ₗ[A] D` through the quotient `V ⧸ M`. Field-agnostic; identical to the finite-dimensional
helper used in Theorem 3.8.1. -/
...
`isNilpotent_sum_of_finiteLength_indecomposable` not all can be nilpotent, so some `f j₀` is a unit
(Fitting dichotomy), yielding mutually inverse-on-both-sides maps `α : W' j₀ → W 0` and
`β : W 0 → W' j₀`. The remaining projection/complement bookkeeping is pure module theory. -/
private lemma krull_schmidt_find_iso_summand_finiteLength
  ...
  have hf_not_all_nilp : ¬ ∀ j, IsNilpotent (f j) := by
    intro hall
    have := isNilpotent_sum_of_finiteLength_indecomposable hW0_indec f hall
    rw [hf_sum] at this
    exact hid_not_nilp this
  push Not at hf_not_all_nilp
  obtain ⟨j₀, hj₀⟩ := hf_not_all_nilp
  have hj₀_bij : Function.Bijective (f j₀) := by
    rcases isNilpotent_or_isUnit_of_finiteLength_indecomposable hW0_indec (f j₀) with hn | hu
    · exact absurd hn hj₀
  ...
  have hφ_bij : Function.Bijective φ := by
    rcases isNilpotent_or_isUnit_of_finiteLength_indecomposable hW'_indec_j₀ φ with hn | hu
...
/-- Auxiliary length-induction for `krull_schmidt_uniqueness_finiteLength`. Universally quantifies
over the module type so the inductive hypothesis applies to the complement `C`. The finite-length
analogue of `krull_schmidt_uniqueness_aux` (Theorem 3.8.1), with `Module.length` as the
strictly-decreasing measure (via `Submodule.length_lt`). -/
...
/-- **Uniqueness half of Krull-Schmidt for finite-length modules** (Remark 3.8.6). Any two
decompositions of a finite-length `A`-module `V` into indecomposable submodules have the same
number of summands, and the summands can be matched up to isomorphism after reindexing. This is the
analogue of the finite-dimensional `Etingof.krull_schmidt_uniqueness` (Theorem 3.8.1), stated with
`Module.length` in place of the `k`-dimension and with no ground field. -/
theorem krull_schmidt_uniqueness_finiteLength [IsArtinian A V] [IsNoetherian A V]
    {n m : ℕ} (W : Fin n → Submodule A V) (W' : Fin m → Submodule A V)
    (hW_indec : ∀ i, Etingof.IsIndecomposable A (W i))
    (hW'_indec : ∀ i, Etingof.IsIndecomposable A (W' i))
