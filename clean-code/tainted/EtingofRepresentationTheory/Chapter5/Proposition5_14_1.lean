...
noncomputable abbrev PermutationModule (n : ℕ) (la : Nat.Partition n) :=
  MonoidAlgebra ℂ (Equiv.Perm (Fin n) ⧸ RowSubgroup n la)
...
private lemma of_smul_single (n : ℕ) (la : Nat.Partition n)
    ...
    (MonoidAlgebra.of ℂ _ g : SymGroupAlgebra n) •
      (MonoidAlgebra.single q c : PermutationModule n la) =
...
theorem Proposition5_14_1_vanishing
  ...
  intro f
  set e : PermutationModule n la := MonoidAlgebra.single (QuotientGroup.mk 1) 1 with he_def
...
Uses invariance to get a_λ * v = |P_λ| • v, and the sandwich lemma (Lemma 5.13.1 dual)
to get a_λ * v = ℓ'(x·a_λ) • c_λ, then combines these with a_λ * c_λ = |P_λ| • c_λ. -/
...
private noncomputable def canonicalHom_ℂ (n : ℕ) (la : Nat.Partition n) :
  ...
  Finsupp.lift (SymGroupAlgebra n) ℂ (Q n la) (canonicalHom_v n la) ∘ₗ
...
-- The nested `MonoidAlgebra` and quotient-module equivariance proof elaborates slowly.
...
private noncomputable def canonicalHom (n : ℕ) (la : Nat.Partition n) :
  ...
  toFun x :=
    ⟨canonicalHom_ℂ n la x, by
      ...
      exact Submodule.smul_of_tower_mem (SpechtModule n la) (x.coeff q)
...
private lemma canonicalHom_apply_identity (n : ℕ) (la : Nat.Partition n) :
    (canonicalHom n la (MonoidAlgebra.single (QuotientGroup.mk 1) 1) : SymGroupAlgebra n) =
...
private lemma equivariant_map_row_invariant (n : ℕ) (la : Nat.Partition n)
    ...
    ∀ p ∈ RowSubgroup n la,
      MonoidAlgebra.of ℂ (G' n) p *
        (f (MonoidAlgebra.single (QuotientGroup.mk 1) 1) : SymGroupAlgebra n) =
        (f (MonoidAlgebra.single (QuotientGroup.mk 1) 1) : SymGroupAlgebra n) := by
  intro p hp
  have h_fix : (MonoidAlgebra.of ℂ _ p : SymGroupAlgebra n) •
      (MonoidAlgebra.single (QuotientGroup.mk (1 : G' n)) (1 : ℂ) : PermutationModule n la) =
...
theorem Proposition5_14_1_diagonal
  ...
  set e : PermutationModule n la := MonoidAlgebra.single (QuotientGroup.mk 1) 1
