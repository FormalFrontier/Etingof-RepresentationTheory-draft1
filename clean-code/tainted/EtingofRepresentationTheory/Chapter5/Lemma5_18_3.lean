...
Part (ii) is formalized in two forms. The algebra-generic statement
`tensorDiagonalSubalgebra_eq_fullDiagonal` works for an arbitrary `k`-algebra `A`
inside the honest tensor-power algebra `⨂[k] (_ : Fin n), A`: the subalgebra
generated by the additive diagonals `Δ_n(a)` equals the one generated by the
multiplicative diagonals `a^{⊗n}`, which by part (i) is the symmetric power
`S^n A`. The representation-theoretic specialization `A = End(V)`,
`diagonalSubalgebra_eq_fullDiagonal`, restates the same content for the diagonal
operators acting on `V^⊗n`.
-/
...
/-!
## Part (ii), algebra-generic form

Lemma 5.18.3(ii) quantifies over *every* `k`-algebra `A`: the `n`-th symmetric
power algebra `S^n A` is generated by the additive diagonals `Δ_n(a)`.

We work inside the honest tensor-power algebra `T = ⨂[k] (_ : Fin n), A`
(Mathlib's `PiTensorProduct` algebra structure). Two subalgebras:

* `tensorDiagonalSubalgebra` is generated by `Δ_n(a) = Σ_i 1 ⊗ ⋯ ⊗ a ⊗ ⋯ ⊗ 1`,
  the *additive* diagonals.
* `tensorFullDiagonalSubalgebra` is generated by `a^{⊗n} = a ⊗ ⋯ ⊗ a`, the
  *multiplicative* diagonals.

By part (i) the multiplicative diagonals `a^{⊗n}` span the symmetric power
`S^n A` (the `S_n`-invariant subspace of `T`), which is a subalgebra, so
`tensorFullDiagonalSubalgebra` *is* `S^n A`. The theorem
`tensorDiagonalSubalgebra_eq_fullDiagonal` therefore says exactly that `S^n A`
is generated as a `k`-algebra by the range of `Δ_n`.

The concrete `A = End_k(V)` statement `diagonalSubalgebra_eq_fullDiagonal`
above is the representation-theoretic specialization: the diagonal
representation `⨂[k] (_ : Fin n), End_k(V) → End_k(V^{⊗n})` carries
`Δ_n(b)` and `b^{⊗n}` to the operators appearing there.
-/
...
/-- `Δ_n(a) = Σ_i 1 ⊗ ⋯ ⊗ a ⊗ ⋯ ⊗ 1`, the additive diagonal of `a ∈ A`
inside the tensor-power algebra `⨂[k] (_ : Fin n), A`. -/
noncomputable def tensorDiag (a : A) : ⨂[k] (_ : Fin n), A :=
  ∑ i : Fin n, singleAlgHom (R := k) (A := fun _ : Fin n => A) i a

/-- `a^{⊗n} = a ⊗ ⋯ ⊗ a`, the multiplicative diagonal of `a ∈ A` inside the
tensor-power algebra `⨂[k] (_ : Fin n), A`. -/
noncomputable def tensorFullDiag (a : A) : ⨂[k] (_ : Fin n), A :=
  tprod k (fun _ : Fin n => a)

/-- The subalgebra of `⨂[k] (_ : Fin n), A` generated by the additive diagonals
`Δ_n(a)`. -/
noncomputable def tensorDiagonalSubalgebra : Subalgebra k (⨂[k] (_ : Fin n), A) :=
  Algebra.adjoin k (Set.range (tensorDiag k A n))

/-- The subalgebra of `⨂[k] (_ : Fin n), A` generated by the multiplicative
diagonals `a^{⊗n}`. By Lemma 5.18.3(i) this is the symmetric power `S^n A`. -/
noncomputable def tensorFullDiagonalSubalgebra : Subalgebra k (⨂[k] (_ : Fin n), A) :=
  Algebra.adjoin k (Set.range (tensorFullDiag k A n))
...
private lemma tensor_diag_le_fullDiag [CharZero k] :
    tensorDiagonalSubalgebra k A n ≤ tensorFullDiagonalSubalgebra k A n := by
  apply Algebra.adjoin_le
  rintro _ ⟨b, rfl⟩
  -- Need: Δ_n(b) = ∑ i, singleAlgHom i b ∈ fullDiag
  by_cases hn : n = 0
  · subst hn; simp only [tensorDiag, Finset.univ_eq_empty, Finset.sum_empty]
  ...
  · have hn' : 0 < n := Nat.pos_of_ne_zero hn
    ...
    have h_mem : ∀ j : Fin (n + 1),
        ∑ m : Fin (n + 1), ((j : ℕ) : k) ^ (m : ℕ) • a m ∈
          (tensorFullDiagonalSubalgebra k A n).toSubmodule := by
      intro j; rw [h_eval]
      exact Algebra.subset_adjoin ⟨((j : ℕ) : k) • (1 : A) + b, rfl⟩
    -- Step 3: all coefficients ∈ fullDiag by Vandermonde
    have h_all := polynomial_coeffs_in_submodule k
      (tensorFullDiagonalSubalgebra k A n).toSubmodule n a h_mem
    -- Step 4: a(n-1) = Δ_n(b)
    have h_target := h_all ⟨n - 1, by omega⟩
    simp only [a] at h_target
    rw [sum_card_pred_eq_sum_erase n hn'] at h_target
...
private lemma tensor_fullDiag_le_diag [CharZero k] :
    tensorFullDiagonalSubalgebra k A n ≤ tensorDiagonalSubalgebra k A n := by
  apply Algebra.adjoin_le
  rintro _ ⟨f, rfl⟩
  ...
  have hpsum : ∀ m, 0 < m → ∑ i, B i ^ m ∈ (tensorDiagonalSubalgebra k A n : Set _) := by
  ...
  set e : ℕ → ⨂[k] (_ : Fin n), A := fun m =>
    ∑ S ∈ (Finset.univ : Finset (Fin n)).powersetCard m,
      S.noncommProd B (fun i _ j _ _ => hcomm i j)
  -- e_n = f^{⊗n}
  suffices heq : e n = tensorFullDiag k A n f by
    change tensorFullDiag k A n f ∈ _
    rw [← heq]
    suffices ∀ m, e m ∈ tensorDiagonalSubalgebra k A n from this n
    intro m
    induction m using Nat.strongRecOn with | ind m ih => ?_
    by_cases hm : m = 0
    · subst hm; simp [e, Finset.powersetCard_zero]
...
/-- **Lemma 5.18.3(ii)** (algebra-generic form). For a `k`-algebra `A` in
characteristic zero, the symmetric power `S^n A`, realized as the subalgebra
of `⨂[k] (_ : Fin n), A` generated by the multiplicative diagonals `a^{⊗n}`,
is generated as a `k`-algebra by the range of the additive diagonal
`Δ_n(a) = Σ_i 1 ⊗ ⋯ ⊗ a ⊗ ⋯ ⊗ 1`. -/
theorem tensorDiagonalSubalgebra_eq_fullDiagonal [CharZero k] :
    tensorDiagonalSubalgebra k A n = tensorFullDiagonalSubalgebra k A n :=
