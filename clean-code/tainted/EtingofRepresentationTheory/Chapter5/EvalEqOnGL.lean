...
This lives in its own Mathlib-only file so that both the polynomial-rep
embedding (`PolynomialRepEmbedding`) and the `det⁻¹`-localization stack
(`DetLocalization` and downstream) can use it without an import cycle: the
localization stack must sit above `PolynomialRepEmbedding` so that the kernel
lemma can be used in the embedding's `detInv_elim_of_polynomial`.
-/

namespace MvPolynomial

-- `MvPolynomial.eq_of_eval_eq_on_gl` is available in Mathlib
-- (`Mathlib/LinearAlgebra/Matrix/GeneralLinearGroup/MvPolynomial.lean`), for any
