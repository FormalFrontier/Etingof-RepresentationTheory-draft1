...
self-contained; they are formalized in this file as theorems:
...
The root-of-unity content of Step 3 is also formalized here:
...
`j`-th power, is formalized here as `character_ringHom_pow`. Its
proof is an eigenvalue computation: `ρ(g)` is semisimple (its minimal
...
formalized here as `character_prod_rat`. It realizes the cyclotomic
...
* `β` is rational: this is the content of Steps 3-4 (Galois invariance in
  `ℚ(ζ_N)`), established below as `character_prod_rat`;
* `β` is an algebraic integer, established here from the fact
...
power. The proof is an eigenvalue computation and needs no cyclotomic-field
...
`β = ∏_{g≠1} χ_V(g)·χ_V(g⁻¹)` (`ringHom_prod_char_inv`), the polynomial form
of `∏_{g≠1} |χ_V(g)|²`. Landing `β ∈ ℚ` from "fixed by every `σ_j`", the
...
/-!
## Composing the remark: `β` integer ⟹ contradiction
...
polynomial form of `∏_{g ≠ 1} |χ_V(g)|²`). We have shown:
...
given it, `not_isIntegral_rat_mem_Ioo` closes the contradiction. The theorem below
takes that bound as a hypothesis (it is established by the main argument of
Problem 5.2.7(b), not this remark) and derives `False`, giving the full logical
structure of the remark as a single theorem.
-/

set_option linter.unusedFintypeInType false in
/-- **Remark 5.2.8, composed.** Let `β = ∏_{g ≠ 1} χ_V(g)·χ_V(g⁻¹)` and let `q : ℚ`
