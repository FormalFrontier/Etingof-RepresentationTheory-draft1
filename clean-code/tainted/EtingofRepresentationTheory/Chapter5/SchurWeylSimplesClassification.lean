...
This file identifies the abstract simple summands `L i` produced by the
equivariant decomposition `glTensorRep_equivariant_schurWeyl_decomposition`
...
`GL_N(k)`-representation. The mathematical content here is the
...
* `formalCharacter_linearIndependent_of_eq_schurPoly`: if the characters of a
  family `L` equal distinct Schur polynomials, they are linearly independent over
  `ℚ`. This is the mechanical half of the strategy: the classification gives
  linear independence via `schurPoly_linearIndependent` and `schurPoly_injective`.

The former classification `schurWeyl_simples_formalCharacter_classification_core`
...
removed: as stated they were false, lacking the nonzero-multiplicity hypothesis
`hSne` (each `S i ≠ 0`), so a degenerate decomposition with `S i₀ = 0` carried an
unconstrained spurious simple `L i₀` whose character need not be a Schur
polynomial. The correct classification is
...
omit [CharZero k] in
/-- **Reduction.** If each character `formalCharacter k N (L i)`
...
identification `char(L i) = schurPoly N λᵢ` (from the classification core
`schurWeyl_simples_formalCharacter_classification_core_general`) into the
`LinearIndependent` statement the Schur-Weyl argument consumes, using
