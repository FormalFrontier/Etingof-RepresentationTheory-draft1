...
/-- **Strengthened Azumaya exchange.** An indecomposable direct summand `X` of a finite biproduct
`⨁ Y` of indecomposables matches a factor `Y k` via an iso *whose forward map is the `k`-component*
`s ≫ biproduct.π Y k`. This refines `indecomposable_summand_iso_factor`, which only records that
some iso exists; the explicit component is what the uniqueness induction needs to make the top-left
entry of a peeled-off binary biproduct invertible (so `Biprod.isoElim` applies). -/
theorem exists_isIso_component {κ : Type} [Fintype κ] (Y : κ → C)
...
theorem indecomposable_summand_iso_factor {κ : Type} [Fintype κ] (Y : κ → C)
    ...
    (s : X ⟶ ⨁ Y) (r : ⨁ Y ⟶ X) (hsr : s ≫ r = 𝟙 X) :
    ∃ k, Nonempty (X ≅ Y k) := by
  obtain ⟨k, hk⟩ := exists_isIso_component Y hY hX s r hsr
  exact ⟨k, ⟨asIso (s ≫ biproduct.π Y k)⟩⟩

set_option linter.unusedFintypeInType false in
/-- **Peeling off one summand.** A finite biproduct `⨁ g` splits as the binary biproduct of the
`i₀`-summand and the biproduct over the complement `{i // i ≠ i₀}`. The `hom`/`inv` are written so
that `biprod.inl ≫ inv = biproduct.ι g i₀` and `hom ≫ biprod.fst = biproduct.π g i₀` hold
definitionally (`peelIso_inv_inl`, `peelIso_hom_fst`), which is what pins the top-left entry of the
peeled iso to the chosen component in the uniqueness induction. -/
noncomputable def peelIso {ι : Type} [Fintype ι] [DecidableEq ι] (g : ι → C) (i₀ : ι) :
...
@[reassoc (attr := simp)]
theorem peelIso_inv_inl {ι : Type} [Fintype ι] [DecidableEq ι] (g : ι → C) (i₀ : ι) :
    biprod.inl ≫ (peelIso g i₀).inv = biproduct.ι g i₀ := by
  simp [peelIso]

@[reassoc (attr := simp)]
theorem peelIso_hom_fst {ι : Type} [Fintype ι] [DecidableEq ι] (g : ι → C) (i₀ : ι) :
    (peelIso g i₀).hom ≫ biprod.fst = biproduct.π g i₀ := by
  simp [peelIso]
...
The cancellation step peels a matched indecomposable summand `Y k₀ ≅ Z m₀` off both sides
(`peelIso`) so that the top-left entry of the resulting binary-biproduct isomorphism is the
invertible component `exists_isIso_component` produced; Gaussian elimination
(`Biprod.isoElim`) then cancels it, leaving the isomorphism of complements. -/
