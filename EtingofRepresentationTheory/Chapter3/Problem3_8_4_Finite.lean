...
`A`-module structures are `bcModRestrict` (`a` acts through `1 ⊗ a`). The map version of
`bcRestrictScalars`: an `L ⊗[K] A`-linear map already commutes with the action of `1 ⊗ a`. -/
...
/-- **Problem 3.8.4(ii), finite-extension case.** For a finite field extension `L / K`, if
`L ⊗[K] V` is a direct summand of `L ⊗[K] W` as `L ⊗[K] A`-modules (a split injection), then `V`
is a direct summand of `W` as `A`-modules.

Restricting scalars along `Algebra.TensorProduct.includeRight` turns the `L ⊗[K] A`-split
injection into an `A`-split injection between the base changes, which the power identification
`L ⊗[K] V ≃ₗ[A] Vⁿ` transports to a split injection `Vⁿ ↪ Wⁿ`; the direct-summand power
cancellation `directSummand_pow_cancel` (Krull-Schmidt over `K`) then exhibits `V` as a direct
summand of `W`. -/
theorem directSummand_of_baseChange_directSummand_finite
  ...
  obtain ⟨i, p, hpi⟩ := h
  set fV := toPiAlinEquiv (A := A) (V := V) (Module.finBasis K L)
  set fW := toPiAlinEquiv (A := A) (V := W) (Module.finBasis K L)
  -- Transport the `A`-linear split injection across the power identifications.
