...
promotes that combinatorial statement to an `sl(2)`-module isomorphism,
following the book's approach (highest-weight vectors + dimension count + distinct Casimir
scalars, avoiding Weyl complete reducibility). The full construction is completed here
in the capstone `clebsch_gordan_module_iso`.
...
construction of the full module isomorphism. -/
...
/-- Every vector in the `k`-th Clebsch–Gordan summand is a Casimir eigenvector of
...
We upgrade the internal decomposition to an `sl(2)`-module isomorphism onto the
...
/-- The intertwiner `⨁_k V_{λ+μ−2k} → V_λ ⊗ V_μ` assembled from the per-summand `cgLieHom`. -/
...
book's approach (highest-weight vectors + dimension count + distinct Casimir scalars, avoiding
