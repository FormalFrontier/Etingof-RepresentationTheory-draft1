...
(Etingof Lemma 5.13.1, CA*RS sandwich version) -/
...
open Etingof Pointwise in
/-- Dual sandwich: a_λ * x * b_λ is a scalar multiple of c_λ = a_λ * b_λ.
There exists a linear functional ℓ' on ℂ[S_n] such that
a_λ * x * b_λ = ℓ'(x) • c_λ for all x.
This is the RS*CA sandwich version (primary for c_λ = a_λ · b_λ convention). -/
