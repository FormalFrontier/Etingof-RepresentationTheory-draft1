...
as partitions of n. The existential carries the module-theoretic
...
summands.  The remaining formalization gap is to replace its current cardinality
reindexing by the genuine Specht classification label and then expose the
multiplicity spaces as the corresponding equivariant Hom spaces.
