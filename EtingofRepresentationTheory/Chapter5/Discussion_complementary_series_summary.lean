...
2. **The class count.** The number of conjugacy classes of `GL₂(𝔽_q)` is `q² − 1`.
   ...
   is recorded here as the `Prop`-valued `foundAllIrreducibles` rather than proved.
...
and number exactly the conjugacy classes, hence exhaust the irreducibles. The
statement (2) is recorded as a `Prop`-valued definition against the group `GL₂(F)`,
paired with the arithmetic identity of (1).
-/
