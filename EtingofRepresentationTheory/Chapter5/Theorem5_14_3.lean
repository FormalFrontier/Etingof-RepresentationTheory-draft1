...
**Book notation.** Etingof (p. 116) writes this formula with `H_m(x)`,
...
book's `H_m` is exactly our `p_m`.

**Note.** The complete homogeneous polynomial `MvPolynomial.hsymm`, `Σ_{|α|=m} x^α`, is
not the book's `H_m`: it allows distributing `m` elements across multiple rows, whereas
each cycle must go to a single row, so it gives the wrong characters. The power sum
`p_m = Σᵢ xᵢᵐ` (the book's `H_m`) correctly enforces this constraint.
...
The complete homogeneous polynomial `hsymm` would be incorrect here: it allows distributing
`m` elements across rows, but each cycle must go entirely to one row. The book's `H_m`
(p. 116) is defined as the power sum `Σⱼ xⱼᵐ = p_m`, not the complete homogeneous
polynomial. -/
