...
The dimension-level shadow of the `SL(V)` isomorphism is a proved `theorem`; the remaining
assertion is recorded with `proof_wanted` (no `sorry`, no axiom), because it is disavowed by the
book:

* `Etingof.algIrrepGL_finrank_constShift`: the dimension-level shadow of `L_λ ≅ L_{λ + c·1ᴺ}`,
  now a proved `theorem`. Twisting by a power of the (one-dimensional) determinant character does
  not change the underlying dimension, so `dim L_λ = dim L_{λ + c·1ᴺ}` (proved via the `+1`
  det-twist identity of Proposition 5.22.2, iterated). This is the statable consequence of the
  `SL(V)` isomorphism at the current infrastructure level: an honest `SL_N`-equivariant isomorphism
