...
-- `imageSubmoduleB c`. The proof body additionally builds a `B`-linear
-- map `Φ' : (↥(S iLam) →ₗ[A] E) →ₗ[B] ↥(imageSubmoduleB c)` whose
-- elaboration repeatedly synthesises `Module B (↥(S iLam) →ₗ[A] E)` via
-- `centralizerModuleHom`. Each synthesis traverses a deep
-- `Subalgebra → Subsemiring → Submodule → Module` chain. Empirically
-- requires ~6.4M / ~3.2M heartbeats.
