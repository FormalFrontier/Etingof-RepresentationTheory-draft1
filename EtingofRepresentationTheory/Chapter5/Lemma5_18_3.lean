...
theorem diagonalSubalgebra_eq_fullDiagonal :
  ...
  · -- ⊆: Each Δ_n(b) ∈ fullDiagonalSubalgebra
    -- Strategy: Δ_n(b) = Σ_i T_i(b) is a k-linear combination of fullDiag generators
    -- {(id + j•b)^⊗n : j = 0,...,n} via Lagrange interpolation.
    --
    -- Key identity (on pure tensors, by multilinearity of PiTensorProduct.tprod):
    -- map (fun _ => id + j•b) (tprod v) = Σ_{S ⊆ Fin n} j^|S| • tprod(b on S, v elsewhere)
    -- So Σ_j c_j • map (fun _ => id + j•b) = Δ_n(b) when Σ_j c_j · j^m = δ_{m,1}.
    -- The Vandermonde matrix [j^m]_{j,m=1..n} is invertible in CharZero.
    --
    -- This requires: MultilinearMap.map_sum_finset for the expansion,
    -- Matrix.det_vandermonde for invertibility, and PiTensorProduct.ext for equality.
    apply Algebra.adjoin_le
    rintro _ ⟨b, rfl⟩
    sorry
  · -- ⊇: Each f^⊗n ∈ diagonalSubalgebra
    -- Strategy: f^⊗n = e_n(T_0(f),...,T_{n-1}(f)), the n-th elementary symmetric
    -- polynomial of pairwise-commuting operators T_i(f) (by singleAction_commute).
    --
    -- By Newton's identity (MvPolynomial.mul_esymm_eq_sum in Mathlib):
    --   m · e_m = Σ_{j=1}^m (-1)^{j+1} · e_{m-j} · p_j
    -- where p_k = Σ_i T_i(f^k) = Δ_n(f^k) ∈ diag.
    --
    -- By induction on m = 1,...,n: each e_m ∈ diag (using e_0 = 1 ∈ diag,
    -- p_k ∈ diag, and dividing by m which is nonzero in CharZero).
    -- In particular e_n = f^⊗n ∈ diag.
    --
    -- The main challenge is proving Newton's identity for operators T_i(f) in the
    -- non-commutative ring End(V^⊗n). Since the T_i pairwise commute, the identity
    -- holds, but Mathlib's MvPolynomial.mul_esymm_eq_sum targets CommRing.
    -- A direct combinatorial proof or factoring through a commutative subalgebra
    -- is needed.
    apply Algebra.adjoin_le
    rintro _ ⟨f, rfl⟩
    sorry
