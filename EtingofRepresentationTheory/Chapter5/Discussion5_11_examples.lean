...
abbrev Z2 : Subgroup S3 := Subgroup.zpowers (Equiv.swap (0 : Fin 3) 1)

/-- `Z₃ ≤ S₃`, the alternating group `A₃ = ⟨(0 1 2)⟩`. -/
abbrev Z3 : Subgroup S3 := alternatingGroup (Fin 3)
...
lemma gen3_zpowers (x : ↥Z3) : x ∈ Subgroup.zpowers gen3 := by
  have htop : Subgroup.zpowers gen3 = ⊤ := by
    ...
    rw [Nat.card_zpowers, gen3_orderOf, Nat.card_eq_fintype_card]
    decide
...
lemma gen2_zpowers : Subgroup.zpowers gen2 = ⊤ := by
  apply Subgroup.eq_top_of_card_eq
  rw [Nat.card_zpowers, gen2_orderOf, Nat.card_zpowers, swap01_orderOf]
...
lemma Z2_card : Fintype.card ↥Z2 = 2 := by
  rw [← Nat.card_eq_fintype_card, Nat.card_zpowers, swap01_orderOf]

lemma Z3_card : Fintype.card ↥Z3 = 3 := by decide
...
/-- `χ_stdRep` on the square of the 3-cycle (the other 3-cycle) is `−1`. -/
lemma stdRep_char_cycle_sq : stdRep.character ((finRotate 3) ^ 2) = -1 := by
  rw [stdRep_character]
  rw [show fixCard ((finRotate 3) ^ 2) = 0 from by decide]
  norm_num

lemma signRep_char_one : signRep.character 1 = 1 := by
  rw [signRep_character]; simp

lemma signRep_char_cycle : signRep.character (finRotate 3) = 1 := by
  rw [signRep_character, signHom_cycle]
...
theorem indZ2_trivPlus_decomp :
  ...
  · -- S ≅ trivRep : multiplicity 1 = 0 + 1
    ...
    have hL : (finrank ℂ (S ⟶ FDRep.of (Etingof.Definition5_8_1 Z2 (charRep (1 : ↥Z2 →* ℂˣ)))) : ℂ)
        = 1 := by
      rw [hLsum]; simp only [hc, trivRep_character]
      rw [invOf_smul_eq_iff, Z2_card, smul_eq_mul]; norm_num
    have hR : finrank ℂ (S ⟶ stdRep ⊞ trivRep) = 1 := by
      rw [finrank_hom_biprod, FDRep.finrank_hom_simple_simple S stdRep,
        FDRep.finrank_hom_simple_simple S trivRep,
        if_neg (fun h => trivRep_not_iso_stdRep ⟨e.symm ≪≫ h.some⟩), if_pos ⟨e⟩]
    rw [hR]; exact_mod_cast hL
  · -- S ≅ signRep : multiplicity 0 = 0 + 0
    obtain ⟨e⟩ := h
    have hc := FDRep.char_iso e
    have hL : (finrank ℂ (S ⟶ FDRep.of (Etingof.Definition5_8_1 Z2 (charRep (1 : ↥Z2 →* ℂˣ)))) : ℂ)
        = 0 := by
      rw [hLsum]; simp only [hc, signRep_char_one, signRep_char_swap]
      rw [invOf_smul_eq_iff, Z2_card, smul_eq_mul]; norm_num
    have hR : finrank ℂ (S ⟶ stdRep ⊞ trivRep) = 0 := by
      rw [finrank_hom_biprod, FDRep.finrank_hom_simple_simple S stdRep,
        FDRep.finrank_hom_simple_simple S trivRep,
        if_neg (fun h => signRep_not_iso_stdRep ⟨e.symm ≪≫ h.some⟩),
        if_neg (fun h => trivRep_not_iso_signRep ⟨(e.symm ≪≫ h.some).symm⟩)]
    rw [hR]; exact_mod_cast hL
  · -- S ≅ stdRep : multiplicity 1 = 1 + 0
    ...
    have hL : (finrank ℂ (S ⟶ FDRep.of (Etingof.Definition5_8_1 Z2 (charRep (1 : ↥Z2 →* ℂˣ)))) : ℂ)
        = 1 := by
      rw [hLsum]; simp only [hc, stdRep_char_one, stdRep_char_swap]
      rw [invOf_smul_eq_iff, Z2_card, smul_eq_mul]; norm_num
    have hR : finrank ℂ (S ⟶ stdRep ⊞ trivRep) = 1 := by
      rw [finrank_hom_biprod, FDRep.finrank_hom_simple_simple S stdRep,
        FDRep.finrank_hom_simple_simple S trivRep, if_pos ⟨e⟩,
        if_neg (fun h => trivRep_not_iso_stdRep ⟨(e.symm ≪≫ h.some).symm⟩)]
    rw [hR]; exact_mod_cast hL
