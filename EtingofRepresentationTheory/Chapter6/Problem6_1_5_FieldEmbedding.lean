...
This is step 2(b) of Etingof Problem 6.1.2, the field-embedding step that
connects step 2(a) (a Zariski-dense orbit) to Problem 6.1.1 (the
...
The reusable field-embedding lemma, phrased so that the geometric input,
injectivity of the comorphism (i.e. Zariski-density of the orbit), is the
...
* `exists_field_embedding_of_injective`: the general form. An injective
...
* `exists_field_embedding_of_injective_mvPolynomial`: the polynomial-action case
  (`B = k[g₁,…,g_M]`), where the orbit map is a morphism and the comorphism lands
  in the polynomial ring directly (Problem 6.1.2 for a single `GLₘ` acting by an
...
* `exists_field_embedding_of_injective_isLocalization`: the principal-open case
  ...
  is the form needed for `W(m)` with the `G(m)`-action. The principal-open
  function-field identification is `isFractionRing_of_isDomain_of_isLocalization`.
* `dim_le_of_injective_comorphism` and its specialisations: the dimension bound
  `N ≤ M`, obtained by applying Problem 6.1.1 to the field embedding.

The remaining geometric input, constructing the concrete orbit-map comorphism
`φ*` on `repSpace`/`repGroup` and proving its injectivity from density of the
orbit, is provided in `Problem6_1_5_OrbitComorphism` and
`Problem6_1_5_OrbitInjective`, and consumed by
`exists_field_embedding_of_injective_isLocalization`.
-/
...
/-- **The field-embedding lemma (general form).** Let `B` be a domain `k`-algebra
...
/-- **The field-embedding lemma (polynomial-action case).** If the comorphism lands
in the polynomial ring `k[g₁,…,g_M]` itself (the orbit map is a morphism, as for a
single `GLₘ` acting by an algebraic representation, Problem 6.1.2 proper), an
...
/-- **The field-embedding lemma (principal-open case).** When the base-change action
...
theorem exists_field_embedding_of_injective_isLocalization
  ...
  -- Surface `SMul`/`Module B K` from `algBK` explicitly.
...
/-- **Problem 6.1.2(b).** An injective comorphism `k[x₁,…,x_N] → B` into a domain
