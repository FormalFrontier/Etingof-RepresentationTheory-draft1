...
/-- **Leaf at a branch vertex (two-branch case).** In a connected acyclic affine Dynkin diagram
with all degrees `≤ 3` and exactly two branch (degree-3) vertices `v, w`, some leaf (degree-1
vertex) `ℓ` is adjacent to `v` or to `w`.

*Proof.* A tree has a leaf `u`; deleting it yields a **finite** Dynkin diagram
(`affine_delete_leaf_isDynkin`). Were `u` adjacent to neither `v` nor `w`, both would retain
degree 3 in the deletion (`hsubdeg`), giving two distinct degree-3 vertices in a finite Dynkin
diagram — impossible by `dynkin_unique_degree_three`. Hence `u` neighbours `v` or `w`.

This is the affine-specific structural input to the two-branch reindexing
`affine_tree_two_branch_iso`: the leaf it produces is the one deleted in the delete-leaf →
finite `branch_classification` → reattach reduction. -/
lemma affine_two_branch_has_leaf {n : ℕ} (adj : Matrix (Fin n) (Fin n) ℤ)
    ...
    ∃ ℓ, Etingof.vertexDegree adj ℓ = 1 ∧ (adj v ℓ = 1 ∨ adj w ℓ = 1) := by
  classical
  -- The two `vertexDegree` definitions are definitionally equal; work with `Etingof.vertexDegree`.
  ...
  obtain ⟨u, hu_deg⟩ : ∃ u, Etingof.vertexDegree adj u = 1 := by
    ...
    -- ≥ 4 vertices, so `Fin (k+1)` is nontrivial.
    have hk3 : 3 ≤ k := by
      have hcnt : Etingof.vertexDegree adj v ≤ (Finset.univ.erase v).card := by
        unfold Etingof.vertexDegree
        apply Finset.card_le_card
        intro x hx
        simp only [Finset.mem_filter, Finset.mem_univ, true_and] at hx
  ...
  have hsubdeg : ∀ (x : Fin (k + 1)) (x' : Fin k), u.succAbove x' = x →
    ...
    change (Finset.univ.filter
      (fun j : Fin k => (adj.submatrix u.succAbove u.succAbove) x' j = 1)).card = 3
    rw [← Finset.card_image_of_injective _ Fin.succAbove_right_injective, himg, hcardN]
  -- `v, w ≠ u` since `u` is a leaf but `v, w` are branch vertices.
  have hvu : v ≠ u := by rintro rfl; rw [hu_deg] at hv; omega
  have hwu : w ≠ u := by rintro rfl; rw [hu_deg] at hw; omega
  -- `u` is adjacent to `v` or `w`: otherwise both survive as degree-3 vertices in the finite
  -- deletion, contradicting `dynkin_unique_degree_three`.
  have hadj_vw : adj u v = 1 ∨ adj u w = 1 := by
    by_contra hcon
    push_neg at hcon
    obtain ⟨hvU, hwU⟩ := hcon
    obtain ⟨v', hv'⟩ := Fin.exists_succAbove_eq hvu
    obtain ⟨w', hw'⟩ := Fin.exists_succAbove_eq hwu
    have hdv' := hsubdeg v v' hv' hv hvU
    have hdw' := hsubdeg w w' hw' hw hwU
    have hveqw : v' = w' := dynkin_unique_degree_three hDsub v' w' hdv' hdw'
    exact hvw (by rw [← hv', ← hw', hveqw])
  refine ⟨u, hu_deg, ?_⟩
  rcases hadj_vw with h | h
  · exact Or.inl (by rw [hsymm' v u]; exact h)
  · exact Or.inr (by rw [hsymm' w u]; exact h)
