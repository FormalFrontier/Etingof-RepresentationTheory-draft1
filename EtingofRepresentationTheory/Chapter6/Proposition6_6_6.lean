...
private theorem Etingof.reflFunctorMinus_mkQ_ker
  ...
  have h_di : inst_dec i i = .isTrue rfl := by
    ...
    | .isTrue _ => rfl
    | .isFalse h => exact absurd rfl h
  -- Use suffices to keep equation homogeneous (both sides under
  -- same Decidable.casesOn), avoiding motive type mismatch.
  suffices h : ∀ z :
...
private noncomputable def Etingof.equivAt_eq_source
  ...
  have f_surj : Function.Surjective f := by
    ...
    have Phi_lof : ∀ (b : @Etingof.ArrowsInto Q instR i) (w : β_in b),
      ...
      intro b w
      show (DirectSum.toModule _ _ _ _) (DirectSum.lof _ _ _ b w) = _
    ...
    have h_Phi_f_ds : ∀ v, Phi (f_ds v) =
      ...
      conv_lhs =>
        arg 2; ext b
        rw [show DirectSum.lof k _ (fun b => ρ_minus.obj b.fst) b = DirectSum.lof k _ β_in b from rfl,
            Phi_lof]
      ...
      show _ = (@Etingof.QuiverRepresentation.sourceMap k _ Q inst ρ i _) v
      delta Etingof.QuiverRepresentation.sourceMap
      simp only [LinearMap.sum_apply, LinearMap.comp_apply]
    ...
    have Phi_of_ne : ∀ (c d : @Etingof.ArrowsInto Q instR i)
      ...
      intro c d w hcd
      show (Phi (DirectSum.lof k _ β_in d w) : Π₀ _, _) _ = _
    ...
    have Phi_of_eq : ∀ (c : @Etingof.ArrowsInto Q instR i)
      ...
      intro c w
      show (Phi (DirectSum.lof k _ β_in c w) : Π₀ _, _) _ = _
...
private theorem Etingof.equivAt_eq_source_naturality
  ...
  have h_sym := (@Etingof.equivAt_eq_source k _ Q inst_dec inst i hi ρ _ _ _ hinj).symm_apply_apply x
  simp only
  revert h_sym x e
  ...
  -- h_sym has expanded LinearEquiv structures. Use dsimp to reduce:
  -- - e(x) = surjInv(x) (LinearMap.inverse has toFun := g)
  -- - e.symm(y) = codRestrict(f_ds)(y) (LinearEquiv.symm swaps toFun/invFun)
  dsimp at h_sym ⊢
  -- Still have (codRestrict...).inverse surjInv applied to x — reduce it
  ...
  -- h_val : f_ds(surjInv(x)) = ↑x  (where f_ds = ∑ lof ∘ equivAt_ne⁻¹ ∘ mapLinear)
  -- Rewrite ↑x in the goal using h_val: f_ds(surjInv(x)) = ↑x
  rw [← h_val]
  -- Goal: equivAt_ne(component_b(f_ds(v))) = mapLinear(e)(v) for v = surjInv(x)
  -- Mathematical argument:
  -- 1. f_ds(v) = ∑_c lof(c)(equivAt_ne⁻¹(mapLinear(arrow_c)(v)))
  -- 2. component_b(f_ds(v)) = equivAt_ne⁻¹(mapLinear(arrow_b)(v))
  --    (by DirectSum.component extracting from sum of singletons)
  -- 3. equivAt_ne(equivAt_ne⁻¹(mapLinear(arrow_b)(v))) = mapLinear(arrow_b)(v)
  -- 4. arrow_b = e (by reversedArrow_ne_eq_eq_ne_twice)
  --
  -- BLOCKER: Quiver instance pollution in the match/Decidable.casesOn branch
  -- prevents stating h_comp (component extraction lemma) because Lean
  -- synthesizes `inst` instead of `reversedAtVertex Q i` for ArrowsInto.
  -- See issue #1724 for full diagnosis.
  sorry
