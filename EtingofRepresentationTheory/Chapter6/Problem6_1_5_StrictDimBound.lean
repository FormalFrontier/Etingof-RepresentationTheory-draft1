...
This is the abstract algebra core of the `−1` strict refinement of the
orbit-dimension bound: the strict analogue of
...
## Transcendence via the scaling action
...
The application supplies `K = Frac(k[τ])`, `f = ι ∘ orbitComorphism`,
...
application supplies only the injective image hom `f`, the
