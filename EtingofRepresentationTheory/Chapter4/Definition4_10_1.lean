...
the factorization theorem (Theorem 4.10.2). Note that the book's matrix `X_G`
(entries `x_{gᵢgⱼ}`, a forward product) differs from the matrix of the regular
representation operator `L(x) = ∑ x_g ρ(g)` (entries `x_{gᵢgⱼ⁻¹}`) by a column
permutation, hence by a sign in the determinant; the proof of Theorem 4.10.2
tracks this sign explicitly.
-/
