...
This file contains the `A₅` character table `chiA5`, its orthonormality, the conjugacy-class
index machinery `classIdxA5`, the three integer-character representations and the exterior
square `Λ²(ℂ⁴)`, related to the tabulated `Q5` values.
-/
...
/-- The tabulated `A₅` characters are orthonormal, the golden-ratio entries contributing
`√5` terms that cancel.  Combined with the fact that `A₅` has exactly 5 conjugacy classes
(`A5_conj_classes`), this shows the five rows are the distinct irreducible characters of
`A₅`. (Etingof Example 4.8.1) -/
...
/-! ### `A₅` representations: trivial `ℂ`, the 4-dim `ℂ⁴`, and the 5-dim `ℂ⁵`

Three of the five rows of `chiA5` are realised here as the characters (traces) of
...
Simplicity of `ℂ⁴` and `ℂ⁵` follows from `FDRep.simple_iff_char_is_norm_one` via the
character-norm sum over the 60 elements; the three rows are pairwise distinct characters,
hence the three representations are pairwise non-isomorphic. -/
...
5-cycle classes are separated by a conjugacy test against `classRepA5 3`. -/
...
Combine with `isConj_iff` to recover `IsConj (classRepA5 (classIdxA5 g)) g`. -/
lemma classIdxA5_spec (g : G) : ∃ c : G, c * classRepA5 (classIdxA5 g) * c⁻¹ = g := by
  revert g; decide

set_option maxRecDepth 8000 in
set_option maxHeartbeats 4000000 in
/-- The five conjugacy classes have sizes `1, 20, 15, 12, 12`, matching `sizesA5`. -/
