...
6. **Check statement integrity (anti-vacuity).** A `sorry`-free proof of a vacuous statement is *not* a formalization, and vacuity is semantic, not just a literal `True`. Read the book's statement, then apply the decision test: *would the Lean statement still be provable if its key object were replaced by something trivial?* If yes — because the conclusion holds for any inhabitant (`Set.Nonempty`, `x = x`), an existential is unconstrained, or an index may be empty — it fails, with status `statement_vacuous` (never `sorry_free`).
7. **Check statement fidelity (no silent weakening).** A declaration existing under the item's name does not satisfy coverage. Against the book's statement, check the Lean statement asserts *every* conjunct and adjective ("irreducible", "distinct", "two-sided", the factor permutation, the bimodule structure); a true, `sorry`-free theorem proving something strictly weaker still fails (e.g. equal composition-series length but not the factor isomorphism). The statement must match its docstring, not merely sit under a faithful one. Record per conjunct in `claim_coverage`; a dropped conjunct is a gap (issue + `derived` item), not `covered_elsewhere`.
...
2. **Per-claim coverage check.** For each candidate, decide `covered_elsewhere` / `genuine_gap` / `non_formalizable` / `trivial` against the *actual* Lean — comparing declarations, signatures, and docstrings, not just names, since coverage is frequently folded into a neighbouring theorem. **`covered_elsewhere` requires fidelity, not mere existence** (apply the Stage 3.2 step 6–7 tests): a named declaration that is vacuous or proves a strictly weaker statement is a `genuine_gap`, the exact case this audit must not wave through. Use a different model for this judge than for the finder, to reduce shared blind spots. Genuine gaps become `accepted` `derived` items (see the items.json schema in Stage 1.6) with GitHub issues; the rest are recorded with a rejection status.
