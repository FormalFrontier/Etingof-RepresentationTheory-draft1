...
- **Follow the book's dependency chain.** If the book says "the result follows from Lemma X.Y.Z", find that lemma in the project and use it in your proof. Even if the lemma still has a `sorry`, Lean accepts it as an axiom — so you can build on it and complete your proof. A sorry'd dependency is not a blocker; it's exactly how top-down development works. Never conclude that a proof is "blocked on missing Mathlib infrastructure" when the book's proof uses an earlier result from the same book.
