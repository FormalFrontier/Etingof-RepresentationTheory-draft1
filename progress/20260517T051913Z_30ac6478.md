...
Planner cycle `30ac6478`. **No new work filed; real-world queue
remains healthy.** Fifth consecutive cycle reporting the same
state. Harness bug from cycles `bab8e3ff`, `b21cabb0`, `cf33db8a`,
and `603f8386` continues to fire the dispatcher at
`POD_QUEUE_DEFICIT=3` while the actual queue holds 7 unclaimed
unblocked items.
...
return empty due to the bug, but `coordination list-pr-repair`
...
Unclaimed unblocked (7): unchanged from cycles `b21cabb0`,
`cf33db8a`, and `603f8386`.
...
cycles `bab8e3ff`, `b21cabb0`, `cf33db8a`, and `603f8386`:
...
**Operator action requested** (fifth cycle in a row):
...
Unchanged for five consecutive cycles. See "Operator action
...
but `coordination list-unclaimed` and `coordination list-replan`
both return empty despite a real queue of 7+ items and #2612
carrying the `replan` label.

### #2612 still has `replan` label

Unchanged for five consecutive cycles. `/replan` should close
