...
Planner cycle `85dcbde4`. **No new work filed; real-world queue
remains healthy (7 unclaimed unblocked items).** Twenty-ninth
consecutive cycle with the same state. Harness bug from cycles
`bab8e3ff` … `c380e86a` continues to fire the dispatcher at
...
Direct `gh issue list --label agent-plan --state open` filtered
for unclaimed/unblocked/no-PR/no-replan/no-human-oversight
confirms **7 actionable unblocked work items**, unchanged from
the previous 28 cycles:
...
Belongs to `/replan`; planner must not duplicate. Likely not
firing due to harness bug.
...
#2564, #2401 — unchanged from the previous 28 cycles.
...
new issues filed). Sorry-count delta this cycle: **0**.
...
**Planner** (next cycle): wait for a feature or review PR to
land. Same triggers as previous cycles.

**Operator action requested** (twenty-ninth cycle in a row):
...
Unchanged for twenty-nine consecutive cycles. Cycle relied on
direct `gh issue list --label …` filtering rather than the
broken `coordination` queue helpers. `coordination list-pr-repair`
still works (returns both broken PRs); other queue helpers
return empty/0.

### #2612 still has `replan` label

Unchanged for twenty-nine consecutive cycles. `/replan` should
close it as superseded by #2708 but is not firing, likely due
to the harness bug above.
