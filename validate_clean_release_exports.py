...
def build_currency_errors(release: Path) -> list[str]:
    ...
    if completed.returncode == 3:
        ...
        errors.append(f"cannot verify build currency: lake exited {completed.returncode}: {tail}")
    return errors
...
def main() -> None:
    ...
    errors.extend(build_currency_errors(release))
